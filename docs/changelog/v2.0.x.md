...
## v2.1.0-beta.5 - 2026-04-25

### Added

- **DeepSeek V4 Flash (haiku tier)**: Introduced `deepseek-v4-flash` as the lite, non-thinking DeepSeek V4 variant. Aliases: `ds-f`, `deepseek-v4-flash`. Routed via the proxy when Claude Code issues a haiku-tier request while switched to DeepSeek.
- **Claude-tier → provider-model mapping (DeepSeek + ZhiPu/Z.AI)**: The proxy now reads `claudeTierMap` from `models-registry.json` and rewrites Claude-tier requests transparently. Defaults:
  - DeepSeek: `opus → deepseek-v4-pro (effort=max)`, `sonnet → deepseek-v4-pro (effort=high)`, `haiku → deepseek-v4-flash (fast path)`.
  - ZhiPu / Z.AI: `opus → glm-5.1`, `sonnet → glm-5-turbo`, `haiku → glm-4.5-air`.
- **GLM-4.5 Air**: Added to the ZhiPu and Z.AI provider rosters as the haiku-tier model. New alias `gair`.
- **Proxy `resolveEffectiveRoute` + `resolveClaudeTier`**: New helpers in `src/proxy/routing/model-resolver.ts` return `{ model, effort? }` for a given request, enabling per-request tier-aware routing without env-var injection.

### Changed

- **DeepSeek Sanitizer (V4 split paths)**: `sanitizeDeepSeekV4` now accepts `{ effort }` and picks between:
  - **Thinking path** (V4 Pro / opus / sonnet): force `thinking.type=enabled`, inject `output_config.effort` from the tier resolver (`max` or `high`), keep placeholder thinking blocks.
  - **Fast path** (V4 Flash / haiku): drop top-level `thinking`, strip thinking content blocks and unsupported types — mirrors the old `deepseek-chat` behaviour.
- **Switched / Directive handlers**: `switched.ts` now calls `resolveEffectiveRoute` and forwards the resolved `effort` to the sanitizer. `directive.ts` derives effort from the target model (`deepseek-v4-pro` → max, `deepseek-v4-flash` → fast path).
- **Statusline, menubar fallback, `/omc-switch`, `omc cc` proxy picker, README (EN/zh-CN)**: refreshed to advertise DeepSeek V4 Pro + V4 Flash, the new GLM tier lineup, and the proxy-side tier mapping.

### Removed

- **Legacy DeepSeek names (`deepseek-chat` / `deepseek-reasoner`)**: Hard-removed from aliases, statusline display maps, `omc cc` proxy picker, and `/omc-switch` docs. The upstream API sunsets these identifiers on 2026-07-24; the proxy no longer forwards or recognises them.

---

## v2.1.0-beta.4 - 2026-04-25

### Added

- **DeepSeek V4 Pro Preset**: Upgraded the DeepSeek preset to the new unified V4 model (`deepseek-v4-pro`). V4 consolidates the old `deepseek-chat` / `deepseek-reasoner` split into a single thinking model where thinking mode is a per-request toggle. See [思考模式](https://api-docs.deepseek.com/zh-cn/guides/thinking_mode).
  - Alias map: `ds`, `ds-r`, `deepseek`, `deepseek-reasoner` all resolve to `deepseek-v4-pro`.
  - New aliases: `deepseek-v4`, `deepseek-v4-pro`.
  - Registry, menubar, statusline, CLI, docs and MCP tool descriptions all updated.

### Changed

- **DeepSeek Sanitizer (V4)**: Replaced the dual `sanitizeDeepSeekChat` / `sanitizeDeepSeekReasoner` paths with a single `sanitizeDeepSeekV4`:
  - Forces `{"thinking": {"type": "enabled"}}` on the top-level body.
  - Injects `{"output_config": {"effort": "max"}}` so Claude Code workloads always use the full reasoner strength.
  - Continues to replace Claude's signed thinking blocks with empty DeepSeek-compatible placeholders (V4 still requires a thinking block on every assistant message when thinking is on).
