...
- **`/omc-switch` Command**: Removed `+1` request padding — proxy handles overhead via `skipInitialRequests`
- **MCP Server**: Added `SLASH_COMMAND_OVERHEAD = 2` constant; switch state now includes `skipInitialRequests` field
- **Proxy State**: `decrementAndCheck()` respects `skipInitialRequests` — decrements skip counter before touching `requestsRemaining`

## v1.4.0-beta.1 - 2026-01-29
...
- **OAuth Proxy Support**: Proxy now works with Claude Code OAuth sessions (no `ANTHROPIC_API_KEY` required)
...
- `ProxyAuthConfig` type now includes `authMode` field
- Settings merger supports `UserPromptSubmit` hook type
- Build system includes `memory-awareness.ts` hook
