...
- **Proxy Auth Auto-Init**: `oh-my-claude cc` proxy now auto-initializes auth config on startup. Previously, passthrough requests to Anthropic failed with 502 `"Proxy auth not configured"` because `proxy-auth.json` was never created (the removed `proxy enable` command used to handle this). The proxy server now calls `initializeAuth()` at startup, auto-detecting oauth vs api-key mode.
