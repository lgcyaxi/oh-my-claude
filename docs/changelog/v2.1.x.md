...
- **Daily Narrative Memory IDs**: `summarize_memories` now auto-detects dates from titles (e.g., "Daily Narrative: 2026-02-14" → ID `2026-02-14-daily-narrative` instead of `2026-02-18-daily-narrative-2026-02-14`). Also supports explicit `createdAt` parameter override.

- **Proxy Auth Auto-Init**: `oh-my-claude cc` proxy now auto-initializes auth config on startup. Previously, passthrough requests to Anthropic failed with 502 `"Proxy auth not configured"` because `proxy-auth.json` was never created after `proxy enable` was removed in v2.0. The proxy server now calls `initializeAuth()` at startup, auto-detecting oauth vs api-key mode.

- **Doctor UI UX Pro Max Detection**: `doctor --detail` now checks both global (`~/.claude/skills/`) and project-local (`.claude/skills/`) paths for UI UX Pro Max, fixing false "not installed" reports.

- **Setup-Tools Install Path**: `setup-tools` now runs `uipro-cli init` from `$HOME` so the skill installs to `~/.claude/skills/ui-ux-pro-max/` (global) instead of the current working directory.

- **Dev Workflow**: `install-local` script now includes `npm link` so the global `oh-my-claude` binary stays current during development.

- **Stale Error Messages**: Updated proxy auth error message and `omc-switch.md` prerequisites to reference `oh-my-claude cc` instead of the removed `proxy enable`/`proxy start` commands.
