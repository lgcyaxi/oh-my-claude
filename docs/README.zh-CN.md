...
OpenCode 是原生协作者目标。对于协作者控制，统一使用 `coworker_task(action=”send” | “review” | “diff” | “fork” | “approve” | “revert” | “cancel” | “status” | “recent_activity”, ...)`。OpenCode 支持显式 `agent` / `provider_id` / `model_id` 覆盖。Codex 通过官方 [openai/codex-plugin-cc](https://github.com/openai/codex-plugin-cc) 插件提供（`omc install` 自动安装）。对于需要持续多轮切换供应商的工作，继续使用 `oh-my-claude cc` 的代理模式。
...
oh-my-claude 提供一键终端配置，针对 AI 编程会话进行了优化（WezTerm 和 tmux）。

<details>
<summary>查看 WezTerm 和 tmux 配置详情</summary>
