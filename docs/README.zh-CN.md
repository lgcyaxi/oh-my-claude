...
**功能：**
- **浏览会话** — 查看所有项目的 Claude Code 对话历史，支持搜索过滤和聊天式对话查看器
- **按会话切换模型** — 选择任意活跃代理会话，独立切换其模型
- **管理模型** — 添加、编辑或删除任意供应商的模型（无需手动编辑 JSON 文件）
- **监控代理** — 查看活跃代理会话的健康状态、运行时间和请求数
- **查看供应商** — 检查供应商配置状态、API 密钥状态和模型数量
...
- 会话浏览器直接读取 JSONL 文件，结合 `sessions-index.json` 元数据
- 侧边栏导航：仪表盘、会话、模型、供应商、切换
