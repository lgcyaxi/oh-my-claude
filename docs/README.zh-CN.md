...
![oh-my-claude 控制面板](assets/dashboard.png)

## 特性

- **多供应商 MCP 服务器** - 支持 DeepSeek、智谱 GLM、MiniMax、Kimi、Aliyun、Ollama、OpenRouter、OpenAI 的后台任务执行
- **OAuth 认证** - 一键登录 MiniMax、Kimi 和 Aliyun — 无需 API 密钥
- **并发后台任务** - 支持多智能体并行运行，可配置并发限制
- **专业化智能体工作流** - 预配置的专业智能体（Sisyphus、Oracle、Hephaestus、Librarian 等）
- **原生协作者运行时** - OpenCode 跨平台查看器、9 项操作、定向差异审查和实时状态。Codex 通过官方 [openai/codex-plugin-cc](https://github.com/openai/codex-plugin-cc) 插件提供（自动安装）
...
- **终端配置** - 一键配置 tmux，跨平台剪贴板支持（Windows 使用 psmux）
...
# OpenRouter（免费模型：nemotron-3-super）
...
支持 OAuth 的供应商可以免 API 密钥使用（OpenAI、MiniMax、Kimi、Aliyun）。

<details>
<summary>查看 OAuth 设置命令</summary>
...
oh-my-claude 提供快捷操作命令（`/omcx-*`）和智能体激活命令（`/omc-*`）。

<details>
<summary>查看所有斜杠命令</summary>
...
<details>
<summary>查看分段、预设和 CLI 控制</summary>
...
**可用分段：** `model`、`git`、`directory`、`context`、`session`、`output-style`、`mode`、`mcp`、`memory`、`proxy`、`usage`、`preferences`
...
oh-my-claude 内置多个输出样式预设（engineer-professional、agent、concise-coder、teaching、review），可自定义 Claude Code 的响应行为。使用 `style set <名称>` 切换，`style create <名称>` 创建自定义样式。

<details>
<summary>查看所有样式和 CLI 命令</summary>
...
<details>
<summary>查看记忆工具、时间线、嵌入和文件格式</summary>
...
<details>
<summary>查看所有模型快捷名、路由详情和代理 CLI</summary>
...
| `dr` | deepseek | deepseek-v4-pro *(V4 Pro，直连时思考强度=max)* |
| `ds-f` | deepseek | deepseek-v4-flash *(V4 lite / haiku 层，fast path)* |
| `g51` | zhipu | glm-5.1 |
...
> **旧名硬移除：** `deepseek-chat` / `deepseek-reasoner` 于 2026-07-24 下线，仅保留上表中的 DeepSeek V4 入口。

**Claude 层级映射（DeepSeek + 智谱/Z.AI）：**

一旦会话切换到 DeepSeek 或智谱/Z.AI，Claude Code 自身的层级请求会被代理透明改写：

| Claude 层级 | DeepSeek                          | GLM（智谱 / Z.AI） |
|-------------|-----------------------------------|--------------------|
...
无需设置 `ANTHROPIC_DEFAULT_*_MODEL` 环境变量 — 映射完全由代理侧完成。

**通过 CLI**（会话 ID 支持前缀匹配）：
```bash
oh-my-claude proxy switch                      # 显示会话和可用模型
oh-my-claude proxy switch 505a GLM-5           # 将会话 505a... 切换到 GLM-5
oh-my-claude proxy switch 505 deep             # 前缀匹配：deepseek-v4-pro
...
独立控制面板位于 `http://localhost:18920/web/` — 随 `omc cc` 自动启动，也可手动管理：

```bash
omc proxy dashboard         # 启动控制面板（端口 18920）
omc proxy dashboard --stop  # 停止控制面板
```

**功能：**
- **浏览会话** — 查看对话历史，重命名（手动 + AI 驱动），删除，清理空会话
- **管理记忆** — 浏览 OMC 项目记忆 + CC 自动记忆，内联编辑、删除
- **管理偏好** — 查看、添加（全局/项目作用域）、删除偏好规则
- **AI 工具** — 批量 AI 会话重命名，记忆操作（压缩/日报/摘要/清理）采用两阶段 分析 → 执行 流程
- **按会话切换模型** — 选择任意活跃代理会话，独立切换其模型
- **管理模型** — 添加、编辑或删除任意供应商的模型
- **监控代理** — 查看活跃代理会话的健康状态、运行时间和请求数
- **明暗主题** — 切换深色、浅色或自动（跟随系统）模式

<details>
<summary>技术细节</summary>

- React 19 + Vite + Tailwind CSS 单页应用，构建到 `dist/proxy/web/`
- 独立控制面板服务器（`dashboard.ts`，端口 18920）— 仅控制 API，无代理端口
- 首次 `omc cc` 会话启动时自动启动
- 9 个页面：仪表盘、会话、OMC 记忆、记忆、模型、供应商、切换、偏好、设置
- 按会话模型切换通过转发 API（`/api/instances/:controlPort/switch`）
- 记忆/偏好 CRUD API + AI 驱动操作（压缩、日报、摘要、清理）
- 所有 AI 功能默认使用 zhipu/glm-5.1，排除 Anthropic（控制面板在 CC 外运行）

</details>

## 终端配置

oh-my-claude 提供一键 tmux 配置，针对 AI 编程会话进行了优化。Windows 用户可通过 `winget install psmux` 安装 psmux 获得 tmux 兼容性。

<details>
<summary>查看 tmux 配置详情</summary>

### tmux

```bash
oh-my-claude terminal-config tmux        # 写入 ~/.tmux.conf
oh-my-claude terminal-config tmux --force # 覆盖已有配置
oh-my-claude terminal-config tmux --show  # 预览但不写入
```

**主要配置：** 50k 滚动缓冲、鼠标模式、256 色、零转义延迟、vi 复制模式。跨平台剪贴板自动检测：`pbcopy`（macOS）、`clip.exe`（Windows/WSL）、`xclip`/`xsel`（Linux）。

**Windows：** 通过 `winget install psmux` 安装 tmux。所有平台共享同一套配置。

</details>

## 智能体工作流

全部 11 个智能体统一为原生 Task 工具智能体。通过提示词中嵌入的 `[omc-route:provider/model]` 路由指令实现自动路由。

<details>
<summary>查看所有智能体和路由详情</summary>
...
# 自动更新检查（所有命令自动执行）
# CLI 自动检查更新并在有新版本时显示提示横幅
# 使用以下环境变量抑制检查：NO_UPDATE_CHECK=1 npx @lgcyaxi/oh-my-claude <command>
# 注意：`update`、`install`、`uninstall` 命令会自动跳过更新检查
...
# 终端配置
npx @lgcyaxi/oh-my-claude terminal-config tmux        # 写入 tmux 配置 (~/.tmux.conf)
npx @lgcyaxi/oh-my-claude terminal-config tmux --force # 覆盖已有配置
