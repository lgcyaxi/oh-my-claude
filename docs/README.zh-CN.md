...
| Analyst | qwen3.5-plus | 指令 → Aliyun |
| Librarian | glm-5 | 指令 → 智谱 |
| Navigator | kimi-for-coding | 指令 → Kimi |
| Hephaestus | kimi-for-coding | 指令 → Kimi |
| Document-Writer | MiniMax-M2.5 | 指令 → MiniMax |
| Frontend-UI-UX | gemini-3-pro | 指令 → Google |
| @kimi | kimi-for-coding | 指令 → Kimi |
| @mm-cn | MiniMax-M2.5 | 指令 → MiniMax CN |
| @deepseek | deepseek-chat | 指令 → DeepSeek |
| @deepseek-r | deepseek-reasoner | 指令 → DeepSeek |
| @qwen | qwen3.5-plus | 指令 → Aliyun |
| @zhipu | glm-5 | 指令 → 智谱 |
...
| 智能体 | 供应商（指令路由） | 模型 | 角色 |
|--------|-------------------|------|------|
| **Oracle** | Anthropic（直通） | claude-sonnet-4-6 | 深度推理、架构设计、调试 |
