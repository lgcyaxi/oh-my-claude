...
代理运行时，通过浏览器访问 `http://localhost:18911/web/` 即可使用控制面板。

**功能：**
- **管理模型** — 添加、编辑或删除任意供应商的模型（无需手动编辑 JSON 文件）
- **切换模型** — 一键切换供应商/模型，实时状态更新
- **监控代理** — 查看代理健康状态、运行时间、请求数和活跃会话
- **会话聚合** — 控制面板聚合所有运行中 `omc cc` 实例的实时会话，显示运行时间、请求数和供应商/模型信息
- **查看供应商** — 检查供应商配置状态、API 密钥状态和模型数量

**使用方式：**
```bash
omc cc          # 自动启动代理
# 打开 http://localhost:18911/web/
```
...
- 直接由代理控制服务器（端口 18911）提供服务 — 无需额外基础设施
- 后端 API：`/api/registry`（模型 CRUD）、`/api/config`（供应商状态）
- 侧边栏导航包含 4 个页面：仪表盘、模型、供应商、切换
- 可扩展设计 — v2 将添加会话管理（浏览/查看/归档对话记录）
