...
### Latest: v2.2.14-beta.8

- **Robustness batch — 23 fixes (13 HIGH + 10 MEDIUM) from the post-beta.7 audit**, grouped into 8 surgical clusters. One new utility (`src/shared/fs/file-lock.ts`) provides reusable `withFileLock*`, `atomicWriteJson/Text`, and `loadJsonOrBackup` primitives that the rest of the clusters use. No broad refactors; every change is local and preserves existing behaviour on the happy path
- **State-file concurrency** — every RMW on `settings.json`, `~/.claude.json`, preferences, preference-awareness cache, session state, `proxy-switch.json`, `proxy-instances.json`, `proxy-sessions.json`, `TIMELINE.md` now runs under an advisory lock + atomic rename. Corrupt state files are backed up to `${path}.corrupt-<unix>.bak` instead of being silently overwritten with defaults (generalising the `SettingsCorruptError` pattern from beta.5)
- **Daemon / dashboard PID lifecycle** — introduced `DAEMON_PID_FILE` alongside `DASHBOARD_PID_FILE`, added `kind: 'daemon' | 'dashboard'` to `startDaemon` / `startDashboard` / `stopDaemon` / `isRunning` / `getPid`, and both spawn paths now use `openSync(pidPath, 'wx')` for atomic PID claim with stale-PID recycle. Fixes the case where starting a dashboard silently clobbered the full-proxy PID
- **Install-path polish** — extracted `DIST_MARKER_REL` to `src/cli/utils/paths.ts`; `update.ts`, `beta-channel.ts`, and `scripts/prepare.cjs` now all agree on the canonical `dist/cli/cli.js` probe (update.ts used to check `dist/cli.js` which never existed, same bug class as the beta.6 beta-channel fix)
- **Memory correctness** — `cacheEmbedding` is finally wired: `indexMemoryWithEmbeddings` eagerly embeds newly-saved memory chunks (gated by `memory.embedding.onWrite`, default `true`) so first `recall` after save is already hybrid-ready. Hybrid search now reports `capabilityTier` vs `executedTier` (collapses to `'fts5'` when no vectors were available even though the system was hybrid-capable). Added `validateMemoryId` path-traversal guard on `getMemory` / `updateMemory` / `deleteMemory` / `createMemory`
- **Provider client cache** — `getProviderClient` now invalidates the cache when `oh-my-claude.json` or `auth.json` mtime changes; `saveAuthStore` explicitly bumps mtime via `utimesSync` so byte-identical re-saves still trigger invalidation. The exported `clearClientCache()` no longer has zero callers
- **Credential permissions** — `scripts/minimax-login.ts`, `scripts/kimi-login.ts`, `scripts/aliyun-login.ts` now write via new shared `writeSecretFile` helper (POSIX `0o600`), matching `src/shared/auth/store.ts` behaviour
- **Proxy stream + request hardening** — `response.failed` (Codex responses API) now emits an Anthropic-shaped `error` event instead of a clean `message_stop` so SDK retry semantics fire; `proxy.maxBodyBytes` (default 4 MiB) enforced before JSON parse with `413 Payload Too Large`; `AbortSignal.any([clientSignal, timeout])` propagates client-disconnect into upstream `fetch` so dropped clients free the TCP connection; OAuth refresh has a 30s `AbortSignal.timeout`; `thinking: {type: 'disabled'}` on DeepSeek fully honoured even when other heuristics would promote to thinking mode; `auto-rotate.ts` AI call respects the hook-wide deadline (no more 45s overruns of the 20s budget) and `.rotation-log.jsonl` is trimmed at 1000 lines

### Previous: v2.2.14-beta.7
