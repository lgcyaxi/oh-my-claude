...
- **Terminal Configuration** - `oh-my-claude wezterm-config` / `tmux-config` for one-command terminal setup with zsh auto-detection, cross-platform clipboard, and split-pane bridge layout
- **WezTerm Config** - Preset `~/.wezterm.lua` (50k scrollback, copy mode, quick select, WebGpu, pane splitting). Auto-detects zsh in Git Bash (priority: zsh > Git Bash > PowerShell)
- **Cross-Platform Clipboard** - `tmux-config` auto-detects clipboard: pbcopy (macOS), clip.exe (Windows/WSL), xclip/xsel (Linux). Mouse drag → system clipboard
- **WezTerm Split-Pane Bridge** - `bridge up codex opencode` creates `|CC|codex,opencode|` layout in WezTerm tabs (matching tmux behavior)
- **CC Tab-vs-Window** - `oh-my-claude cc` inside WezTerm opens a new tab instead of a new window (detects `$WEZTERM_PANE`)
- **Kimi Proxy Fix** - Fixed 400 errors when switching to Kimi (`tool_reference` stripped). Thinking blocks/config preserved
- **CC-to-CC Bridge** - Spawn Claude Code instances as bridge workers (`bridge up cc`), each with its own proxy session. Auto-switch to cheap external models (DeepSeek, ZhiPu, MiniMax) via `--switch`. Multi-instance support (`cc:2`, `cc:3`). Response capture via pane-output polling
- **Event-Driven Bridge** - Bridge auto-close, default pane-right layout, signal file notifications, `wait_for_tasks` MCP tool, per-session request tracking. Codex session discovery fixed for v0.101+ nested date dirs
- **Bridge-Aware Sisyphus** - Sisyphus now checks bridge status and routes tasks (scaffolding, refactoring, code gen, visual-to-code) to bridge CLI tools when available, with fallback routing when bridge is inactive
