...
export default function SessionsPage() {
  ...
  async function handleAiRename(sessionId: string) {
    if (!selectedFolder) return;
    setAiRenamingId(sessionId);
    try {
      await aiRenameSession(selectedFolder, sessionId, 'zhipu', 'glm-5');
      reloadSessions();
    } catch { /* ignore */ }
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        {s.firstPrompt && (
                          <button
                            onClick={(e) => {
                              e.preventDefault();
                              handleAiRename(s.sessionId);
                            }}
                            disabled={aiRenamingId === s.sessionId}
                            title="AI Rename"
                            className="p-1 text-text-tertiary hover:text-accent text-[10px] disabled:animate-pulse"
                          >
                            {aiRenamingId === s.sessionId ? '...' : '✦'}
                          </button>
                        )}
                        <button
                          onClick={(e) => {
