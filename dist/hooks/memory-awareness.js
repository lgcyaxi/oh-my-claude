...
function getMemoryCount(projectCwd) {
  ...
  if (projectCwd) {
    ...
    if (canonicalRoot && canonicalRoot !== projectCwd) {
      ...
      if (existsSync6(canonicalMemDir) && !seenDirs.has(canonicalMemDir)) {
...
function getClaudeNativeMemory(projectCwd) {
  ...
  try {
    ...
    if (!existsSync6(claudeProjectsDir))
      return null;
    ...
    if (existsSync6(memoryFile)) {
      const content = readFileSync6(memoryFile, "utf-8").trim();
  ...
  const scanDir = (baseDir, scope) => {
    for (const subdir of ["notes", "sessions"]) {
      ...
      for (const file of files) {
        try {
          const content = readFileSync6(join6(dir, file), "utf-8");
...
function scanCompletionSignals() {
  ...
  try {
    ...
    for (const file of files) {
      ...
      try {
        const signal = JSON.parse(readFileSync6(filePath, "utf-8"));
