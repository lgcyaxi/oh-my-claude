...
var init_minimax = __esm(() => {
  CREDS_PATH = join17(homedir10(), ".claude", "oh-my-claude", "minimax-creds.json");
});
...
var CREDS_PATH2;
var init_kimi = __esm(() => {
  CREDS_PATH2 = join18(homedir11(), ".claude", "oh-my-claude", "kimi-creds.json");
});
...
var init_aliyun = __esm(() => {
  CREDS_PATH3 = join19(homedir12(), ".claude", "oh-my-claude", "aliyun-creds.json");
});
...
function buildAgentDefaults() {
  const agents = {};
  for (const [name, agent] of Object.entries(import_models_registry.default.agents)) {
    const a = agent;
...
function buildCategoryDefaults() {
  const categories = {};
  for (const [name, cat] of Object.entries(import_models_registry.default.categories)) {
    categories[name] = {
...
async function handleSwitch(req, sessionId, sessionTag, corsHeaders) {
  ...
  const displayModel = body.model.startsWith(`${body.provider}/`) ? body.model : `${body.provider}/${body.model}`;
  console.error(`[control]${sessionTag} Switched to ${displayModel}` + (warning ? ` [${warning}]` : ""));
...
function getRegistryPath() {
  return join4(homedir4(), ".claude", "oh-my-claude", "models-registry.json");
}
...
function writeRegistry(registry) {
  ...
  writeFileSync3(tmpPath, JSON.stringify(registry, null, "\t") + `
`, "utf-8");
...
var REGISTRY_DIR = join5(homedir5(), ".claude", "oh-my-claude");
var REGISTRY_FILE = join5(REGISTRY_DIR, "proxy-instances.json");
var LOCK_FILE = join5(REGISTRY_DIR, "proxy-instances.lock");
...
var PROJECTS_DIR = join6(homedir6(), ".claude", "projects");
...
async function resolveProjectPath(folder) {
  const dirPath = join6(PROJECTS_DIR, folder);
  try {
    const entries = await readdir(dirPath, { withFileTypes: true });
    const candidates = [];
    for (const entry of entries) {
      if (entry.isFile() && entry.name.endsWith(".jsonl")) {
        candidates.push(join6(dirPath, entry.name));
      }
    }
    if (candidates.length === 0) {
      for (const entry of entries) {
        if (!entry.isDirectory() || entry.name === "memory")
          continue;
        const subagentsDir = join6(dirPath, entry.name, "subagents");
        try {
          const subFiles = await readdir(subagentsDir);
          for (const sf of subFiles) {
            if (sf.endsWith(".jsonl")) {
              candidates.push(join6(subagentsDir, sf));
              break;
            }
          }
        } catch {}
...
async function readSessionIndex(folder) {
  try {
    const indexPath = join6(PROJECTS_DIR, folder, "sessions-index.json");
...
async function scanJsonlFiles(folder) {
  ...
  for (const entry of entries) {
    ...
    if (seenIds.has(entry.name))
      continue;
    const sessionDir = join6(dirPath, entry.name);
    try {
...
var PROJECTS_DIR2 = join9(homedir7(), ".claude", "projects");
...
function resolveOmcProjectMemoryPath(id) {
  ...
  if (!projectRoot)
    return null;
  return join9(projectRoot, ".claude", "mem", subdir, `${noteId}.md`);
}
...
async function findJsonlFile(projDir) {
  ...
  if (rootJsonl)
    return join9(projDir, rootJsonl);
  for (const name of files) {
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(name))
      continue;
    const subDir = join9(projDir, name);
    const subFiles = await readdir3(subDir).catch(() => []);
    const jsonl = subFiles.find((n) => n.endsWith(".jsonl"));
    if (jsonl)
      return join9(subDir, jsonl);
    if (subFiles.includes("subagents")) {
      const agentFiles = await readdir3(join9(subDir, "subagents")).catch(() => []);
      const agentJsonl = agentFiles.find((n) => n.endsWith(".jsonl"));
      if (agentJsonl)
        return join9(subDir, "subagents", agentJsonl);
    }
  }
  return null;
}
...
async function findOmcProjectMemDirs() {
  ...
  try {
    ...
    for (const f of folders) {
      if (!f.isDirectory())
        continue;
      const projDir = join9(PROJECTS_DIR2, f.name);
...
function resolveMemoryPath(scope, id) {
  ...
  if (fileId === "MEMORY") {
    return join9(PROJECTS_DIR2, folder, "memory", "MEMORY.md");
  }
  return join9(PROJECTS_DIR2, folder, "memory", "notes", `${fileId}.md`);
}
...
async function readMemoryFiles(dir, scope) {
  ...
  for (const file of files) {
    if (!file.endsWith(".md"))
      continue;
    try {
      const raw = await readFile4(join10(dir, file), "utf-8");
...
async function collectMemoryEntries(targetProject, fullContent) {
      ...
      if (fullContent) {
        ...
        for (const e of entries) {
          all.push({
            ...
            type: e.type,
            created: e.created,
            filePath: join11(dir.replace(/\\/g, "/").includes("/sessions") ? dir : dir, `${e.id}.md`),
            dir
          });
        }
      }
  ...
  if (!targetProject || globalOnly) {
    for (const sub of ["notes", "sessions"]) {
      const dir = join11(GLOBAL_MEMORY_DIR, sub);
      if (fullContent) {
        all.push(...await readMemoryFilesWithPaths(dir));
      } else {
        const entries = await readMemoryFiles(dir, "global");
  ...
  for (const sub of ["notes", "sessions"]) {
    ...
    for (const file of files) {
      if (!file.endsWith(".md") || file === "TIMELINE.md")
        continue;
      try {
        const raw = await readFile5(join12(dir, file), "utf-8");
        const { meta, content } = parseFrontmatter(raw);
        const titleMatch = content.match(/^#?\s*(.+)$/m);
        const rawTags = Array.isArray(meta.tags) ? meta.tags : [];
        const rawConcepts = Array.isArray(meta.concepts) ? meta.concepts : [];
...
> ... truncated (${lines.length - maxLines + 1} lines omitted)`);
    await writeFile3(join12(memRoot, "TIMELINE.md"), truncated.join(`
`), "utf-8");
  } else {
    await writeFile3(join12(memRoot, "TIMELINE.md"), lines.join(`
`), "utf-8");
  }
}
...
async function handleDailyOperation(controlPort, body, targetProject, corsHeaders) {
  ...
  for (const date of datesToProcess) {
    ...
    try {
      ...
      const narrativePath = join13(targetDir, `${narrativeId}.md`);
...
${result.content}`;
  ...
  for (const entry of sessionEntries) {
    const memRoot = join13(entry.dir, "..");
...
`);
        ...
        const newPath = join13(targetDir, `${newId}.md`);
...
async function handleListMemories(corsHeaders) {
  const globalNotes = await readMemoryFiles(join14(GLOBAL_MEMORY_DIR, "notes"), "global");
  const globalSessions = await readMemoryFiles(join14(GLOBAL_MEMORY_DIR, "sessions"), "global");
  const globalEntries = [
    ...globalNotes,
    ...globalSessions
  ];
...
async function findJsonlFile2(projDir) {
  ...
  const rootJsonl = files.find((n) => n.endsWith(".jsonl"));
  if (rootJsonl)
    return join15(projDir, rootJsonl);
  for (const name of files) {
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(name))
      continue;
    const subDir = join15(projDir, name);
    const subFiles = await readdir7(subDir).catch(() => []);
    const jsonl = subFiles.find((n) => n.endsWith(".jsonl"));
    if (jsonl)
      return join15(subDir, jsonl);
    if (subFiles.includes("subagents")) {
      const agentFiles = await readdir7(join15(subDir, "subagents")).catch(() => []);
      const agentJsonl = agentFiles.find((n) => n.endsWith(".jsonl"));
      if (agentJsonl)
        return join15(subDir, "subagents", agentJsonl);
    }
  }
  return null;
}
async function findProjectPrefs() {
  ...
  try {
    ...
    for (const f of folders) {
      if (!f.isDirectory())
        continue;
      const projDir = join15(PROJECTS_DIR3, f.name);
...
function getWebDirs() {
  const dirs = [];
  dirs.push(join16(process.cwd(), "dist", "proxy", "web"));
  const installed = join16(homedir9(), ".claude", "oh-my-claude", "dist", "proxy", "web");
  dirs.push(installed);
  if (typeof __dirname !== "undefined") {
    dirs.push(join16(__dirname, "web"));
  }
  return dirs;
}
...
function serveWebAsset(assetPath) {
  ...
  const filePath = join16(webDir, normalized);
