...
- **Proxy-only** (`category: ["proxy"]`, external APIs via MCP):
  Provider-specific tasks now use bridge worker roles (`code`/`audit`/`docs`/`design`) via `bridge_send`.
...
- `runtime/` — runtime & infra: up, down, status, status-bridge, codex, opencode, pref
...
- `src/mcp/bridge/send.ts` - Bridge send flow (request dispatch and response handling)
- `src/mcp/tasks/run.ts` - Task runner execution flow
- `src/mcp/shared/types.ts` - Shared MCP domain types
- `src/assets/agents/index.ts` - Agent registry: nativeAgents, proxyAgents, bridgeAgents
...
- `docs/guides/codex-app-server.md` - Codex app-server wire protocol, lifecycle, and bridge integration guide
