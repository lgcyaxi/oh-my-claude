...
export class RpcTransport {
	...
	processLine(line: string): void {
		...
		// JSON-RPC request from server (has numeric `id` and `method`)
		if (typeof msg.id === 'number' && msg.method) {
			void this.onRequest(msg.id, msg.method, msg.params);
			return;
		}

		// JSON-RPC response (has numeric `id` but no `method`)
		if (typeof msg.id === 'number') {
			...
			if (pending) {
				...
				if (msg.error) {
					pending.reject(
						new Error(
							`RPC ${msg.method ?? msg.id}: ${msg.error.message}`,
						),
					);
				} else {
