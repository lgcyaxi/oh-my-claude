...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const wasmPath = join(homedir(), ".claude", "oh-my-claude", "mcp", "sql-wasm.wasm");
      ...
      switch (embeddingProvider) {
        ...
        case "openrouter": {
          embeddingAvailable = !!process.env.OPENROUTER_API_KEY;
          embeddingDetail = embeddingAvailable ? `openrouter/${embeddingModel}` : "openrouter (OPENROUTER_API_KEY not set)";
          break;
        }
        case "none":
      ...
      if (detail) {
        ...
        if (wasmExists) {
          try {
            const wasmStats = require("node:fs").statSync(wasmPath);
        ...
        if (indexDbExists) {
          try {
            const dbStats = require("node:fs").statSync(indexDbPath);
        ...
        if (embeddingProvider === "custom") {
          ...
          // Connectivity test for custom provider
          if (apiBase) {
            try {
              const url = apiBase.replace(/\/+$/, "").replace(/\/embeddings$/, "") + "/embeddings";
              const response = execSync(
                `curl -s -o /dev/null -w "%{http_code}" -X POST "${url}" -H "Content-Type: application/json" -d '{"model":"${model ?? "text-embedding-3-small"}","input":["test"]}' --max-time 5`,
                { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] }
              ).trim();
              const statusCode = parseInt(response, 10);
              if (statusCode === 200) {
                console.log(`    Connectivity: ${c.green}OK (HTTP 200)${c.reset}`);
              } else {
                console.log(`    Connectivity: ${c.yellow}HTTP ${statusCode}${c.reset}`);
              }
            } catch {
          ...
          console.log(`    OPENROUTER_API_KEY: ${process.env.OPENROUTER_API_KEY ? `${c.green}(set)${c.reset}` : `${c.red}(not set)${c.reset}`}`);
          console.log(`    Model: ${embeddingModel}`);
        }
    ...
    if (options.fixMem) {
      ...
      const { copyFileSync, mkdirSync, statSync: fsStat } = require("node:fs");
      const installDir = join(homedir(), ".claude", "oh-my-claude");
      ...
      if (existsSync(wasmTarget)) {
        ...
        if (!wasmSource) {
          try {
            ...
            const pkgDir = require("node:path").dirname(pkgPath);
      ...
      const memIndexDbPath = join(installDir, "memory", "index.db");
      ...
      if (!existsSync(join(mcpDir, "sql-wasm.wasm"))) {
        ...
        try {
          ...
          if (!indexer.isReady()) {
            ...
            // Show index size
            try {
              const dbStat = fsStat(memIndexDbPath);
      ...
      if (fixEmbProvider === "none") {
        ...
        if (!apiBase) {
          ...
          try {
            const url = apiBase.replace(/\/+$/, "").replace(/\/embeddings$/, "") + "/embeddings";
            const response = execSync(
              `curl -s -w "\\n%{http_code}" -X POST "${url}" -H "Content-Type: application/json" -d '{"model":"${model}","input":["fix-mem connectivity test"]}' --max-time 10`,
              { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] }
            );
            ...
            if (statusCode === 200) {
              ...
              console.log(`    ${fail(`HTTP ${statusCode} from ${apiBase}`)}`);
            }
          } catch {
        ...
        const keyEnv = fixEmbProvider === "zhipu" ? "ZHIPU_API_KEY" : "OPENROUTER_API_KEY";
