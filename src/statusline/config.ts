...
export const StatusLineConfigSchema = z.object({
	...
	segments: z
		.object({
			...
			bridge: SegmentConfigSchema.default({
				enabled: true,
				position: 1,
				row: 3,
			}),
...
export function getDefaultConfig(
	preset: StatusLineConfig['preset'] = 'standard',
): StatusLineConfig {
...
export function loadConfig(): StatusLineConfig {
	try {
		...
		const hasRowField = parsed.segments
			? Object.values(parsed.segments).some(
					(s: any) => typeof s?.row === 'number',
				)
			: false;

		const validated = StatusLineConfigSchema.parse(parsed);

		return applyPresetToConfig(
			validated as StatusLineConfig,
			rawSegmentKeys,
			hasRowField,
		);
	} catch {
...
export function ensureConfigDir(): boolean {
	try {
		if (existsSync(CONFIG_DIR)) {
			...
			if (!stat.isDirectory()) {
				console.error(
					`[statusline] Config path exists but is not a directory: ${CONFIG_DIR}`,
				);
				return false;
			}
			return true;
...
export function saveConfig(config: StatusLineConfig): void {
	try {
		...
		if (!existsSync(CONFIG_PATH)) {
			console.error(
				`[statusline] Config file was not created at: ${CONFIG_PATH}`,
			);
		}
	} catch (error) {
		console.error(
			`[statusline] Failed to save config to ${CONFIG_PATH}:`,
			error,
		);
	}
}
...
export function ensureConfigExists(
	preset: StatusLineConfig['preset'] = 'full',
): boolean {
	...
	if (!existsSync(CONFIG_PATH)) {
		...
		if (!existsSync(CONFIG_PATH)) {
			console.error(
				`[statusline] Failed to create default config at: ${CONFIG_PATH}`,
			);
			return false;
		}
	}
