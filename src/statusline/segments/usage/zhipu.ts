...
export async function fetchZhiPuQuota(
	...
	try {
		...
		const weeklyTokenLimit = data.data.limits.find(
			(l) => l.type === 'TOKENS_LIMIT' && l.unit === 6,
		);
		const shortTokenLimit = data.data.limits.find(
			(l) => l.type === 'TOKENS_LIMIT' && l.unit !== 6,
		);
		// unit 5 = monthly call limit (TIME_LIMIT)
		const monthlyCallLimit = data.data.limits.find(
		...
		if (monthlyCallLimit) {
			...
			parts.push(
				`m:${Math.round(monthlyCallLimit.percentage)}%/${remaining}`,
			);
		}
		...
		const pct =
			weeklyTokenLimit?.percentage ??
			monthlyCallLimit?.percentage ??
			shortTokenLimit?.percentage ??
			0;
