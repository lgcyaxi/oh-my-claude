...
const BUILD_HASH: string =
	typeof __OMC_BUILD_HASH__ !== 'undefined' ? __OMC_BUILD_HASH__ : 'dev';
...
export function readCache(): UsageCache {
	try {
		...
		const cachedVersion = isNewFormat
			? (parsed as UsageCacheFile).version
			: undefined;
		const cacheData = isNewFormat
			? ((parsed as UsageCacheFile).data ?? {})
			: (parsed as UsageCache);
