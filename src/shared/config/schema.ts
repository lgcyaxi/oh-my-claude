...
type RegistryAgent = (typeof modelsRegistry.agents)[keyof typeof modelsRegistry.agents];

function buildAgentDefaults(): Record<string, { provider: string; model: string; temperature?: number; fallback?: { provider: string; model: string } }> {
  const agents: Record<string, any> = {};
  for (const [name, agent] of Object.entries(modelsRegistry.agents)) {
    const a = agent as RegistryAgent & { temperature?: number; fallback?: { provider: string; model: string } };
...
function buildCategoryDefaults(): Record<string, { provider: string; model: string; temperature?: number }> {
  const categories: Record<string, any> = {};
  for (const [name, cat] of Object.entries(modelsRegistry.categories)) {
    categories[name] = { provider: cat.provider, model: cat.model, temperature: cat.temperature };
  }
  return categories;
}
...
export const OhMyClaudeConfigSchema = z.object({
  ...
  agents: z.record(z.string(), AgentConfigSchema).default(buildAgentDefaults()),

  categories: z.record(z.string(), CategoryConfigSchema).default(buildCategoryDefaults()),
