 ...
 * Used for direct provider access by proxy routing and model switching.
 */
...
export function getProvidersStatus(): Record<
  string,
  { configured: boolean; type: string; apiKeyEnv?: string }
> {
  const config = loadConfig();
  const status: Record<string, { configured: boolean; type: string; apiKeyEnv?: string }> = {};
