...
import { getBridgeWorkerShellCommand } from "./platform";

/**
 * Resolve the WezTerm binary path.
 * On Windows: returns bundled path from oh-my-claude if available, otherwise "wezterm".
 */
function resolveWezterm(): string {
  if (process.platform === "win32") {
    const bundled = join(homedir(), ".claude", "oh-my-claude", "apps", "wezterm", "windows-x64", "wezterm.exe");
...
export class WezTermBackend extends BaseTerminalBackend {
  ...
  private readonly bin = resolveWezterm();

  private getShellWrapper(command: string): string[] {
    return getBridgeWorkerShellCommand(command);
  }
  ...
  async createPane(name: string, command: string, options?: PaneCreateOptions): Promise<PaneId> {
    ...
    if (options?.split && options?.targetPane) {
      ...
      args.push("--", ...this.getShellWrapper(command));
    } else {
      // Default: spawn a new window
      // Shell wrapper keeps the window alive after the command exits, so the
    ...
    const { stdout } = await this.runCommand(this.bin, args, { timeoutMs: 15_000 });
  ...
  async closePane(paneId: PaneId): Promise<void> {
    try {
      await this.runCommand(this.bin, ["cli", "kill-pane", "--pane-id", paneId], {
        timeoutMs: 10_000,
      });
    } catch {
  ...
  async listPanes(): Promise<PaneInfo[]> {
    ...
    try {
      ...
      const { stdout } = await this.runCommandRaw(this.bin, ["cli", "list", "--format", "json"], {
        timeoutMs: 10_000,
      });
  ...
  async injectText(paneId: PaneId, text: string): Promise<void> {
    ...
    await this.runCommand(this.bin, ["cli", "send-text", "--pane-id", paneId, "--no-paste"], {
      timeoutMs: 10_000,
      input: cleanText,
    });
  ...
  async isPaneAlive(paneId: PaneId): Promise<boolean> {
    try {
      const { stdout } = await this.runCommand(this.bin, ["cli", "list", "--format", "json"], {
        timeoutMs: 10_000,
      });
  ...
  async getPaneOutput(paneId: PaneId, _lines: number): Promise<string> {
    try {
      const { stdout } = await this.runCommand(this.bin, ["cli", "get-text", "--pane-id", paneId], {
        timeoutMs: 10_000,
      });
      return stdout;
    } catch {
  ...
  private async ensureWezTermInstalled(): Promise<void> {
    const available = await this.commandExists(this.bin, ["--version"]);
    if (!available) {
      throw new TerminalBackendError({
        message: `wezterm is not installed or not available in PATH (tried: ${this.bin}).`,
        command: this.bin,
        args: ["--version"],
      });
    }
