...
export class CCDaemon extends AIDaemon {
  ...
  constructor(options: CCDaemonOptions) {
    ...
    this.runtimeDir = options.runtimeDir ?? join(homedir(), ".claude", "oh-my-claude", "run", options.config.name.replace(/:/g, "-"));
  }
  ...
  private buildStartupCommand(): string {
    ...
    // Inject -p <provider> flag for pre-switched bridge workers.
    // MUST be inserted right after "cc" subcommand (before -skip and other pass-through args)
    // because commander's passThroughOptions may not parse -p if placed after unknown flags.
    if (this.config.switchProvider && !args.includes("-p") && !args.includes("--provider")) {
      const ccIdx = args.indexOf("cc");
      if (ccIdx >= 0) {
        args.splice(ccIdx + 1, 0, "-p", this.config.switchProvider);
      } else {
        args.push("-p", this.config.switchProvider);
      }
    }
    ...
    // Build env var prefix for bridge workers.
    // Use env -u to fully remove TMUX and WEZTERM_PANE from the environment
    // so Claude Code doesn't try to create new terminal windows/sessions.
    // Empty assignment (TMUX=) keeps the var present as "" which tmux CLI still detects.
    const envVars: string[] = [
      "env", "-u", "TMUX", "-u", "WEZTERM_PANE",
      "OMC_BRIDGE_PANE=1",
      `OMC_BUS_PORT=${process.env.OMC_BUS_PORT ?? "18912"}`,
      `OMC_BRIDGE_WORKER_ID=${this.name}`,
    ];
    // Pass session ID for multi-session bus isolation
    const sessionId = process.env.OMC_SESSION_ID;
    if (sessionId) {
      envVars.push(`OMC_SESSION_ID=${sessionId}`);
    }

    // Use Windows syntax (set VAR=1 &&) when running under cmd.exe, Unix syntax otherwise
    if (process.platform === "win32") {
      const setVars = envVars.map(v => `set ${v}`).join(" && ");
      return `${setVars} && ${cmd}`;
    }
    return `${envVars.join(" ")} ${cmd}`;
  }
