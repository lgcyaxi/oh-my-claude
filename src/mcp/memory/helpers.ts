...
export function afterMemoryMutation(projectRoot: string | undefined): void {
  try { regenerateTimelines(projectRoot); } catch { /* best-effort */ }
}
...
export async function indexNewMemory(
  ...
  try {
    ...
    const memDir = actualScope === "project" && projectRoot
  ...
  try {
    ...
    for (const subdir of ["notes", "sessions"]) {
      if (projectRoot) {
        ...
        if (projDir) indexer.removeFile(join(projDir, subdir, `${id}.md`));
      }
      const globalDir = getMemoryDir();
      if (globalDir) indexer.removeFile(join(globalDir, subdir, `${id}.md`));
    }
  } catch { /* best-effort */ }
}
 ...
 */
export function getSearchDirs(projectRoot: string | undefined): Array<{ path: string; scope: "project" | "global"; projectRoot?: string }> {
