 ...
 * characters, causing "bridge" → ["b","r","i","d","g","e"].
 */
...
export async function handleMemoryTool(
  ...
  switch (name) {
    case "remember": {
      const { content, title, type, tags, concepts, files, scope } = args as {
        content: string;
        title?: string;
        type?: "note" | "session";
        tags?: string[];
      ...
      const result = createMemory(
        { content, title, type, tags: memTags, concepts: normalizeInputTags(concepts), files, scope },
        cachedProjectRoot,
      );
      if (!result.success) {
        return {
      ...
      return {
        content: [{
          ...
          text: JSON.stringify({
            ...
            ...(result.data!.concepts && result.data!.concepts.length > 0 && { concepts: result.data!.concepts }),
            ...(result.data!.files && result.data!.files.length > 0 && { files: result.data!.files }),
            scope: actualScope,
            reason,
            ...(nearDupeInfo && {
    ...
    case "recall": {
      const { query, type, tags, concepts, limit, scope } = args as {
        query?: string;
        type?: "note" | "session";
        tags?: string[];
        concepts?: string[];
        ...
        content: [{
          ...
          text: JSON.stringify({
            ...
            memories: results.map((r) => ({
              ...
              type: r.entry.type,
              tags: r.entry.tags,
              ...(r.entry.concepts && r.entry.concepts.length > 0 && { concepts: r.entry.concepts }),
              score: r.score,
      ...
      return {
        content: [{
          ...
          text: JSON.stringify({
            ...
            type: entry.type,
            tags: entry.tags,
            ...(entry.concepts && entry.concepts.length > 0 && { concepts: entry.concepts }),
        ...
        try {
          ...
          if (entry) {
            ...
            if (entryScope === "project" && cachedProjectRoot) {
              ...
              if (projDir) {
                await indexer.removeFile(join(projDir, subdir, `${id}.md`));
                indexCleaned = true;
              }
            } else {
              await indexer.removeFile(join(getMemoryDir(), subdir, `${id}.md`));
              indexCleaned = true;
            }
      ...
      return {
        content: [{
          ...
          text: JSON.stringify({
            ...
            defaultWriteScope: getDefaultWriteScope(cachedProjectRoot, getConfiguredWriteScope()),
            indexStatus,
            tokenUsage: indexer?.isReady() ? indexer.getTokenStats() : null,
          }),
        }],
      };
    }
