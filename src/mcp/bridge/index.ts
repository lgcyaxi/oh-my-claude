...
export { bridgeToolSchemas } from "./schemas";
  ...
  switch (name) {
    case "bridge_up": {
      if (process.env.OMC_BRIDGE_PANE === "1") {
        return {
          content: [{ type: "text", text: JSON.stringify({ error: "bridge_up is not available inside bridge workers. Complete the task directly." }) }],
          isError: true,
        };
      }
      const { name: workerName, switch_alias } = args as { name: string; switch_alias?: string };

      if (!workerName || !workerName.startsWith("cc")) {
        return { content: [{ type: "text", text: JSON.stringify({ error: "name is required and must start with 'cc'" }) }], isError: true };
      }

      try {
        const { addAIToStateSafe } = await import("../../workers/bridge/state");
        ...
        const sessionEnv = sessionId ? `OMC_SESSION_ID=${sessionId} ` : "";

        // Auto-derive provider from predefined config (cc:zp → zhipu, cc:ds → deepseek, etc.)
        let providerFlag = "";
        try {
          const { DEFAULT_BRIDGE_AI_CONFIGS } = await import("./config");
          const { getBridgeBaseAIName } = await import("./config");
          const cfg = DEFAULT_BRIDGE_AI_CONFIGS[workerName] ?? DEFAULT_BRIDGE_AI_CONFIGS[getBridgeBaseAIName(workerName)];
        ...
        if (backend === "tmux") {
          ...
          try {
            const paneResult = execSync("tmux display-message -p '#D'", { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] }).trim();
            if (paneResult.startsWith("%")) targetPane = paneResult;
          } catch { /* no current pane — fall back to bridge session */ }
          ...
          if (targetPane) {
            ...
            try { execSync(`tmux has-session -t ${bridgeSession}`, { stdio: "pipe" }); } catch {
        ...
        if (!lockResult.added) {
          ...
          return { content: [{ type: "text", text: JSON.stringify({ error: lockResult.error, hint: "Concurrent bridge_up detected — duplicate pane killed" }) }], isError: true };
        }
        ...
        if (!effectiveSwitchAlias) {
          try {
            const { DEFAULT_BRIDGE_AI_CONFIGS, getBridgeBaseAIName } = await import("./config");
            const cfg = DEFAULT_BRIDGE_AI_CONFIGS[workerName] ?? DEFAULT_BRIDGE_AI_CONFIGS[getBridgeBaseAIName(workerName)];
            if (cfg?.switchProvider) {
        ...
        if (effectiveSwitchAlias) {
          ...
          const switchTarget = SWITCH_ALIAS_MAP[effectiveSwitchAlias!];

          if (switchTarget) {
            // Wait for proxy to register, then switch
            const { readProxyRegistry, cleanupStaleEntries } = await import("../../proxy/registry");
            ...
            if (controlPort && sessionId) {
              try {
                const resp = await fetch(`http://localhost:${controlPort}/switch?session=${sessionId}`, {
                  method: "POST",
                ...
                if (resp.ok) {
                  ...
                  // Persist proxy info
                  const { readBridgeState: reread, writeBridgeState } = await import("../../workers/bridge/state");
                  const updated = reread();
                  const aiEntry = updated.ais.find(a => a.name === workerName);
                  if (aiEntry) {
              ...
              switchResult = "Proxy not found after 15s, skipping auto-switch";
            }
          } else {
        ...
        return {
          content: [{ type: "text", text: JSON.stringify({ error: `Failed to start ${workerName}: ${error instanceof Error ? error.message : String(error)}` }) }],
          isError: true,
        };
      }
    }
    ...
    case "bridge_down": {
      if (process.env.OMC_BRIDGE_PANE === "1") {
        return {
          content: [{ type: "text", text: JSON.stringify({ error: "bridge_down is not available inside bridge workers. Complete the task directly." }) }],
          isError: true,
        };
      }
      ...
      try {
        const { readBridgeState, removeAIFromState } = await import("../../workers/bridge/state");
        const state = readBridgeState();
        const entry = state.ais.find(a => a.name === workerName);

        if (!entry) {
          return {
            content: [{ type: "text", text: JSON.stringify({ error: `${workerName} is not running`, running_ais: state.ais.map(a => a.name) }) }],
            isError: true,
          };
        }
        ...
        if (entry.paneId) {
          try {
            ...
            if (entry.terminalBackend === "tmux") {
              execSync(`tmux kill-pane -t ${entry.paneId}`, { stdio: "pipe" });
            } else if (entry.terminalBackend === "wezterm") {
              execSync(`wezterm cli kill-pane --pane-id ${entry.paneId}`, { stdio: "pipe" });
            }
          } catch {
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              stopped: true,
              name: workerName,
              workers_remaining: remaining.ais.map(a => a.name),
        ...
        return {
          content: [{ type: "text", text: JSON.stringify({ error: `Failed to stop ${workerName}: ${error instanceof Error ? error.message : String(error)}` }) }],
          isError: true,
        };
      }
    }
    ...
    case "bridge_status": {
      try {
        ...
        const { execSync: execSyncStatus } = await import("node:child_process");
        const { join } = await import("node:path");
        const { homedir } = await import("node:os");

        const state = readBridgeState();
        const codexStatusPath = join(homedir(), ".claude", "oh-my-claude", "run", "codex-status.json");

        // Liveness check helper: verify tmux/wezterm pane still exists
        function isPaneAlive(paneId: string, backend?: string): boolean {
          try {
            if (backend === "tmux") {
              // list-panes on a specific pane target fails with exit code 1 if pane is dead
              execSyncStatus(`tmux list-panes -t "${paneId}"`, { stdio: "pipe", timeout: 3000 });
              return true;
            } else if (backend === "wezterm") {
              const out = execSyncStatus(`wezterm cli list --format json`, { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"], timeout: 3000 });
              return out.includes(paneId);
            }
          } catch { /* pane not found */ }
          return false;
        }
          ...
          if (isProcBased && ai.name === "codex") {
            try {
              if (existsSync(codexStatusPath)) {
                const sig = JSON.parse(readFileSync(codexStatusPath, "utf-8")) as { state?: string; updatedAt?: number };
                if (sig.updatedAt && Date.now() - sig.updatedAt < 30_000) {
        ...
        for (const name of staleNames) {
          try { removeAIFromState(name); } catch { /* best-effort */ }
        }
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              ...(staleNames.length > 0 ? { pruned: staleNames } : {}),
            }),
          }],
        };
      } catch (error) {
        return {
          content: [{ type: "text", text: JSON.stringify({ error: `bridge_status failed: ${error instanceof Error ? error.message : String(error)}` }) }],
          isError: true,
        };
      }
    }
    ...
    case "bridge_send": {
      if (process.env.OMC_BRIDGE_PANE === "1") {
        return {
          content: [{ type: "text", text: JSON.stringify({ error: "bridge_send is not available inside bridge workers. Complete the task directly." }) }],
          isError: true,
        };
      }
    ...
    case "bridge_dispatch": {
      if (process.env.OMC_BRIDGE_PANE === "1") {
        return {
          content: [{ type: "text", text: JSON.stringify({ error: "bridge_dispatch is not available inside bridge workers. Complete the task directly." }) }],
          isError: true,
        };
      }
    ...
    case "bridge_wait": {
      if (process.env.OMC_BRIDGE_PANE === "1") {
        return {
          content: [{ type: "text", text: JSON.stringify({ error: "bridge_wait is not available inside bridge workers. Complete the task directly." }) }],
          isError: true,
        };
      }
