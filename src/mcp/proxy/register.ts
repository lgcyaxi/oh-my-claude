...
⚠️ switch_model changes ALL session traffic and requires switch_revert. Only use for sustained multi-turn work (3+ interactions). For single tasks, use coworker_task(action="send", ...) or Task tool subagents instead.
...
- deepseek: deepseek-v4-pro (V4 unified, thinking effort=max)
- zhipu: glm-5.1, glm-5
...
**Example**: switch_model(provider="deepseek", model="deepseek-v4-pro")
This routes all subsequent Claude Code requests through DeepSeek V4 Pro (thinking mode, effort=max).
...
Once switched, use /model <name> to switch between any installed Ollama model.`,
			inputSchema: z.object({
				provider: z
					.string()
					.describe(
						'Provider name (deepseek, zhipu, minimax, kimi, aliyun, openrouter, openai, ollama)',
					),
				model: z
					.string()
					.optional()
					.describe(
						'Model name. Optional for ollama (auto-discovered). Examples: deepseek-v4-pro, glm-5.1, glm-5, MiniMax-M2.7, kimi-for-coding, qwen3.5-plus, nvidia/nemotron-3-super-120b-a12b:free, gpt-5.3-codex, llama3.3',
					),
			}),
		},
