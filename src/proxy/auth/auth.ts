...
export async function getProviderAuth(
	...
	if (!baseUrl) {
		throw new Error(
			`Provider "${providerName}" has no base_url configured.`,
		);
	}
	...
	if (OAUTH_PROVIDER_TYPES.has(providerConfig.type)) {
		try {
			const { getAccessToken } =
				await import('../../shared/auth/token-manager');
	...
	if (!apiKeyEnv) {
		throw new Error(
			`Provider "${providerName}" has no api_key_env configured.`,
		);
	}
