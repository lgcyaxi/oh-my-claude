...
export function sanitizeDeepSeekChat(body: Record<string, unknown>): void {
  ...
  if (strippedKeys > 0 || strippedBlocks > 0) {
    console.error(`[sanitize:deepseek-chat] stripped ${strippedBlocks} thinking blocks, ${strippedKeys} top-level keys`);
  }
...
export function sanitizeDeepSeekReasoner(body: Record<string, unknown>): void {
  ...
  if (replaced > 0) {
    console.error(`[sanitize:deepseek-reasoner] replaced ${replaced} thinking blocks`);
  }
