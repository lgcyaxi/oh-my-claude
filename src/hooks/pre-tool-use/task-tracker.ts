...
async function main() {
	...
	const hasResponse =
		toolInput.tool_response || toolInput.tool_output || toolInput.output;
	...
	if (isPreToolUse) {
		...
		const index = taskAgents.agents.findIndex(
			(a) => a.type === displayName,
		);
