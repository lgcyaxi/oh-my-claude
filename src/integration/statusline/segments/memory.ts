 ...
 */
function readTokenStats(): { embeddingCalls: number; searchQueries: number } | null {
  try {
    const statsPath = join(homedir(), ".claude", "oh-my-claude", "memory", "token-stats.json");
    if (!existsSync(statsPath)) return null;
    const raw = readFileSync(statsPath, "utf-8");
    const data = JSON.parse(raw);
    if (typeof data.embeddingCalls === "number" && typeof data.searchQueries === "number") {
      return { embeddingCalls: data.embeddingCalls, searchQueries: data.searchQueries };
    }
    return null;
  } catch {
    return null;
 ...
 * Collect memory data
 */
async function collectMemoryData(context: SegmentContext): Promise<SegmentData | null> {
  try {
