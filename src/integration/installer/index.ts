...
import { existsSync, mkdirSync, writeFileSync, copyFileSync, cpSync, readdirSync, readFileSync, statSync, unlinkSync, rmSync } from "node:fs";
...
export async function install(options?: {
  ...
  if (debug) {
    ...
    console.log(`[DEBUG] src/assets/commands exists: ${existsSync(join(sourceDir, "src", "assets", "commands"))}`);
  ...
  try {
    ...
    if (!options?.skipCommands) {
      try {
        ...
        const srcCommandsDir = join(sourceDir, "src", "assets", "commands");
        if (existsSync(srcCommandsDir)) {
          ...
          const collectMdFiles = (dir: string): string[] => {
            ...
            for (const entry of readdirSync(dir, { withFileTypes: true })) {
              if (entry.isDirectory()) {
                files.push(...collectMdFiles(join(dir, entry.name)));
          ...
          const deprecatedCommands = [
            ...
            "omc-up.md",          // removed in v2.1.3-beta.37 — stale bridge REST API, use bridge CLI
            "omc-down.md",        // removed in v2.1.3-beta.37 — stale bridge REST API, use bridge CLI
            "omc-status-bridge.md", // removed in v2.1.3-beta.37 — merged into /omc-status
          ];
...
`,
    ...
    // 4b2. Install bridge bus server
    try {
      const busDir = join(installDir, "dist", "bridge-bus");
      if (!existsSync(busDir)) {
        mkdirSync(busDir, { recursive: true });
      }

      const builtBusDir = join(sourceDir, "dist", "bridge-bus");
      if (existsSync(builtBusDir)) {
        cpSync(builtBusDir, busDir, { recursive: true });
      }
    } catch (error) {
      // Non-critical — bus is opt-in
      if (debug) console.log(`[DEBUG] Failed to install bridge bus: ${error}`);
    }
    ...
    try {
      ...
      if (existsSync(srcMenubarDir)) {
        ...
        const lfsPointerFiles: Array<{ relativePath: string; destPath: string }> = [];
        ...
        cpSync(srcMenubarDir, menubarDir, {
          ...
          filter: (src) => {
            ...
            if (basename === "node_modules" || basename === "target") return false;

            // Detect LFS pointer files in builds/ — they're not real binaries.
            // A real binary is >1KB; LFS pointers are ~130 bytes.
            if (src.includes(`builds${require("node:path").sep}`) && basename && !basename.includes(".")) {
              try {
                const stat = statSync(src);
                if (stat.isFile() && stat.size < 1024) {
                  const head = readFileSync(src, { encoding: "utf-8" }).slice(0, 40);
                  if (head.startsWith("version https://git-lfs")) {
                    ...
                    lfsPointerFiles.push({ relativePath: `apps/menubar/${rel}`, destPath: dest });
                    if (debug) console.log(`[DEBUG] LFS pointer detected: ${src}`);
                    return false; // Skip copy — will download real binary below
                  }
                }
              } catch { /* stat failed — include file */ }
            }
            return true;
          }
        });
        ...
        if (lfsPointerFiles.length > 0) {
          ...
          for (const { relativePath, destPath } of lfsPointerFiles) {
            ...
            try {
              ...
              if (debug) console.log(`[DEBUG] LFS download URL: ${url}`);
              ...
              if (resp.ok) {
                ...
                if (buffer.length < 1024) {
                  ...
                  result.warnings.push(`Menubar binary download returned invalid data. Run 'oh-my-claude menubar --build' to build locally.`);
                  continue;
                }
                ...
                if (debug) console.log(`[DEBUG] LFS download failed (HTTP ${resp.status}): ${url}`);
                result.warnings.push(`Menubar binary download failed (HTTP ${resp.status}). Run 'oh-my-claude menubar --build' to build locally.`);
              }
            } catch (dlError) {
              if (debug) console.log(`[DEBUG] LFS binary download error: ${dlError}`);
              result.warnings.push(`Menubar binary download failed. Run 'oh-my-claude menubar --build' to build locally.`);
            }
          }
        }
            ...
            for (const platform of readdirSync(buildsDir)) {
              ...
              for (const file of readdirSync(platformDir)) {
                ...
                if (fileStat.isFile() && fileStat.size < 1024) {
                  const head = readFileSync(filePath, { encoding: "utf-8" }).slice(0, 40);
                  if (head.startsWith("version https://git-lfs")) {
                    unlinkSync(filePath);
                    if (debug) console.log(`[DEBUG] Removed stale LFS pointer: ${filePath}`);
                  }
                }
              }
            }
        ...
        if (existsSync(menubarPkgJson) && !existsSync(menubarNodeModules)) {
          try {
    ...
    try {
      const registrySrc = join(sourceDir, "src", "shared", "config", "models-registry.json");
      // Also check dist/ in case we're running from an npm install
      const registrySrcDist = join(sourceDir, "dist", "config", "models-registry.json");
      const registryDest = join(installDir, "models-registry.json");

      const registrySource = existsSync(registrySrc) ? registrySrc : existsSync(registrySrcDist) ? registrySrcDist : null;
      if (registrySource) {
        copyFileSync(registrySource, registryDest);
      }
    } catch (error) {
      // Non-critical — menubar falls back to hardcoded defaults
      if (debug) console.log(`[DEBUG] Failed to install models registry: ${error}`);
    }
