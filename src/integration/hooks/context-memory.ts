 ...
 */
function isCompletionTool(toolName: string, toolInput: Record<string, unknown> | undefined): boolean {
...
async function pollBusForTasks(): Promise<string> {
  ...
  try {
    ...
    const data = (await resp.json()) as {
      tasks: Array<{
        taskId: string;
        mandate: { role: string; scope: string; goal: string; acceptance: string; context?: string };
      }>;
      count: number;
    };
...
async function handleStop(input: HookInput): Promise<HookResponse> {
  ...
  if (!transcriptText && input.transcript_path) {
    ...
    } catch { /* transcript_path may not be readable */ }
  }

  if (transcriptText) {
    const maxLen = 6000;
    const transcript = transcriptText.length > maxLen
  ...
  if (input.last_assistant_message) {
    const msg = input.last_assistant_message.length > 1000
