...
function readBridgeWorkers(): string[] {
  try {
    const stateFile = join(homedir(), ".claude", "oh-my-claude", "bridge-state.json");
    if (!existsSync(stateFile)) return [];
    const parsed = JSON.parse(readFileSync(stateFile, "utf-8")) as { ais?: Array<{ name: string }> };
...
function warnIfNonOmcProxyBaseUrl(sessionId?: string): void {
  ...
  process.stderr.write("⚠️  Proxy not detected. Running natively — bridge features limited.\n");
}
...
const STOP_WORDS = new Set([
  ...
  "they", "them", "what", "which", "who", "whom", "make", "like", "use",
  "please", "help", "want", "need", "get", "let", "also", "new",
  "implement", "fix", "refactor", "add", "update", "create", "debug",
]);
...
async function main() {
  ...
  if (isBridgeWorker && bridgeWorkerId) {
    ...
    try {
      ...
      const resp = await fetch(`http://localhost:${busPort}/tasks/${encodeURIComponent(bridgeWorkerId)}?${sessionParam}`, {
        signal: AbortSignal.timeout(3000),
      });
      if (resp.ok) {
        const data = (await resp.json()) as { tasks: Array<{ taskId: string; mandate: { role: string; scope: string; goal: string; acceptance: string; context?: string } }>; count: number };
        if (data.count > 0) {
          ...
          taskLines.push(`1. Call bridge_event with task_id and type="accepted" to acknowledge`);
          taskLines.push(`2. Complete the task as described above`);
          taskLines.push(`3. Call bridge_event with type="completed" and payload={message, files?, data?}`);
          taskLines.push(`4. If you fail, call bridge_event with type="failed" and payload={error: "reason"}`);
  ...
  if (workerRole) {
    const rolePrompts: Record<string, string> = {
      ...
      general: "You are a bridge worker. Complete tasks directly. Do NOT use bridge_send or spawn sub-workers.",
    };
  ...
  if (!isBridgeMode) {
    try {
      const sessionDir = join(homedir(), ".claude", "oh-my-claude", "sessions");
      const sessionId = input.session_id;
      if (sessionId) {
        const modeFile = join(sessionDir, sessionId, "mode.json");
        if (existsSync(modeFile)) {
          const modeData = JSON.parse(readFileSync(modeFile, "utf-8"));
