...
import { getSessionStatusPath, getSessionTaskAgentsPath, ensureSessionDir, writeCurrentPPID } from "../statusline/session";
...
function logToolObservation(toolName: string, toolInput: Record<string, unknown>, toolOutput: string, projectCwd?: string): void {
  // Skip self-referential tools
  if (toolName === "session-logger" || toolName === "auto-memory" || toolName === "context-memory") return;
...
function saveTaskAgents(data: TaskAgentsData): void {
  try {
    ensureSessionDir();
    writeFileSync(getSessionTaskAgentsPath(), JSON.stringify(data, null, 2));
  } catch { /* ignore */ }
}
...
function updateStatusFile(taskAgents: TaskAgentsData): void {
  try {
    ...
    let status: any = { activeTasks: [], providers: {}, updatedAt: new Date().toISOString() };
    if (existsSync(statusPath)) {
      try { status = JSON.parse(readFileSync(statusPath, "utf-8")); } catch { /* use default */ }
    }

    const thirtyMinutesAgo = Date.now() - 30 * 60 * 1000;
    const taskAgentTasks = taskAgents.agents
      .filter(a => a.startedAt > thirtyMinutesAgo)
      .map(a => ({ agent: `@${a.type}`, startedAt: a.startedAt, model: a.model, isTaskTool: true }));
...
function handleTaskTool(input: PostToolUseInput): string | null {
  ...
  const model = (toolInputData.model as string) || DEFAULT_MODELS[subagentType] || "?";
...
const NOTIFIED_FILE = join(homedir(), ".claude", "oh-my-claude", "notified-tasks.json");
...
function scanCompletionSignals(): string[] {
  ...
  try {
    if (existsSync(NOTIFIED_FILE)) {
      ...
      notified = new Set(Object.entries(data).filter(([_, ts]) => (ts as number) > oneHourAgo).map(([id]) => id));
    } else {
      notified = new Set();
    }
  } catch {
...
function saveMiniNote(toolInput: Record<string, unknown>, projectCwd?: string): void {
  ...
  try {
    ...
    if (projectCwd) {
      ...
      if (gitRoot) {
        notesDir = join(resolveCanonicalRoot(gitRoot), ".claude", "mem", "notes");
      } else {
        notesDir = join(homedir(), ".claude", "oh-my-claude", "memory", "notes");
      }
    } else {
      notesDir = join(homedir(), ".claude", "oh-my-claude", "memory", "notes");
    }
    ...
    writeFileSync(join(notesDir, `auto-commit-${dateStr}-${timeStr}.md`), content, "utf-8");
...
async function main() {
  ...
  if (signalNotifications.length > 0) contextParts.push(...signalNotifications);
