...
## How to delegate
1. Read relevant files for context (Read/Grep/Glob/Bash available).
2. Formulate a complete self-contained task (the worker cannot ask questions).
3. Call \`mcp__oh-my-claude__bridge_send\` with:
   - \`ai_name\`: \`${workerName}\`
   - \`message\`: your complete task description
   - \`wait_for_response\`: \`true\`
4. Return the worker's response.
