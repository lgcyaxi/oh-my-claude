...
export function generateAgentMarkdown(agent: AgentDefinition): string {
	...
	// All agents get full tool access for implementation tasks.
	lines.push(
	...
	// Embed route directive for non-Claude agents.
	// Provider agents use explicit provider/model (no resolution needed).
	// Role agents use model-only (proxy resolves best provider at request time).
	// When no proxy is active, the directive is harmless text — Claude native model is used.
