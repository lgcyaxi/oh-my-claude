...
type BridgeDoctorStatus = { ok: boolean; message: string };

function providerHasReachableBaseUrl(config: OhMyClaudeConfig, providerName: string): boolean {
...
function evaluateBridgeWorkerStatus(
...
export function registerDoctorCommand(program: Command) {
  program
    ...
    .action(async (options) => {
      const { isBetaInstallation, getBetaChannelInfo, checkForNewerBeta } = require("../../../integration/installer/beta-channel");
      ...
      if (detail) {
        ...
        try {
          // Read settings.json directly — `claude mcp list` hangs inside Claude Code sessions
        ...
        try {
          const { validateStatusLineSetup } = require("../../../integration/installer/statusline-merger");
