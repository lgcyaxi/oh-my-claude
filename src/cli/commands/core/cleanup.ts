...
export function registerCleanupCommand(program: Command) {
	program
		...
		.action((options) => {
			const { ok, fail, warn, dimText } = createFormatters();
			...
			if (force) {
				console.log(
					warn('Force mode - all sessions will be cleaned\n'),
				);
			}
				...
				for (const entry of entries) {
					if (entry.isDirectory()) {
						...
						if (dirName.startsWith('pid-')) {
							...
							if (!isNaN(pid) && pid > 0) {
								if (!isProcessRunning(pid)) {
									...
									console.log(
										`  ${dimText(dirName)} - active (PID running)`,
									);
								}
							} else {
								shouldClean = true;
						...
						if (shouldClean) {
							if (dryRun) {
								console.log(
									`  ${warn(dirName)} - would delete (${reason})`,
								);
								totalCleaned++;
							} else {
								try {
									...
									console.log(
										`  ${ok(dirName)} - deleted (${reason})`,
									);
									totalCleaned++;
								} catch (error) {
									console.log(
										`  ${fail(dirName)} - failed to delete`,
									);
								}
							}
						}
			...
			if (existsSync(ppidFile)) {
				...
				try {
					...
					if (
						...
						console.log(
							`  ${dimText('current-ppid.txt')} - active (PID ${pid} running)`,
						);
					} else {
						if (dryRun) {
							console.log(
								`  ${warn('current-ppid.txt')} - would delete (PID ${pid} not running, ${ageMinutes}m old)`,
							);
							totalCleaned++;
						} else {
							rmSync(ppidFile, { force: true });
							console.log(
								`  ${ok('current-ppid.txt')} - deleted (PID ${pid} not running)`,
							);
							totalCleaned++;
						}
					}
					...
					if (dryRun) {
						console.log(
							`  ${warn('current-ppid.txt')} - would delete (unreadable)`,
						);
						totalCleaned++;
					} else {
						rmSync(ppidFile, { force: true });
						console.log(
							`  ${ok('current-ppid.txt')} - deleted (unreadable)`,
						);
						totalCleaned++;
					}
				}
			...
			if (existsSync(debugLog)) {
				...
				if (dryRun) {
					console.log(
						`  ${warn('task-tracker-debug.log')} - would delete (${sizeKb}KB)`,
					);
					totalCleaned++;
				} else {
					rmSync(debugLog, { force: true });
					console.log(
						`  ${ok('task-tracker-debug.log')} - deleted (${sizeKb}KB)`,
					);
					totalCleaned++;
				}
			}
			...
			if (dryRun) {
				console.log(
					`Dry run complete. Would clean ${totalCleaned} item(s).`,
				);
			} else {
				console.log(
					`Cleanup complete. Removed ${totalCleaned} item(s).`,
				);
			}
		});
}
