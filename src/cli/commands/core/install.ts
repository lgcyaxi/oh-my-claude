	...
	program
		...
		.action(async (options) => {
			...
			if (result.commands.skipped.length > 0) {
				console.log(
					'⊘ Skipped commands:',
					result.commands.skipped.join(', '),
				);
			}
			if (result.commands.removed.length > 0) {
				console.log(
					'  ↳ Removed deprecated:',
					result.commands.removed.map((c) => `/${c}`).join(', '),
				);
			}
			...
			if (result.success) {
				try {
					...
					console.log(
						'\n💡 tmux not found. Install it for advanced terminal workflows:',
					);
					console.log('   brew install tmux');
					console.log(
						'   (oh-my-claude works without tmux — this is optional)',
					);
				}
			}
			...
			if (result.success) {
				...
				if (result.mcp.installed || result.mcp.updated) {
					console.log(
						'\n4. Restart Claude Code or open a fresh Claude Code session to reload MCP server changes.',
					);
				}
			}
