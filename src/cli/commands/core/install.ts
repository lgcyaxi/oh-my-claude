...
import { install, uninstall } from "../../../integration/installer";
...
export function registerInstallCommands(program: Command) {
  ...
  program
    ...
    .action(async (options) => {
      ...
      if (result.success) {
        try {
          execSync("which tmux", { stdio: "ignore", windowsHide: true });
        } catch {
          console.log("\n💡 tmux not found. Install it for multi-provider bridge workflows:");
          console.log("   brew install tmux");
          console.log("   (oh-my-claude works without tmux — this is optional)");
        }
      }
