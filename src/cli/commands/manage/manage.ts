...
export function registerManageCommand(program: Command) {
  ...
  const manageCmd = program
    .command("manage")
    .alias("m")
    .description("oh-my-claude manager — pref, memory, config, statusline, terminal-config")
    .action(() => {
      console.log(`${c.bold}oh-my-claude Manager${c.reset}  ${c.dim}(shortcut: omc m)${c.reset}\n`);
      console.log(`  omc m pref        ${c.dim}# Manage preferences${c.reset}`);
      console.log(`  omc m memory      ${c.dim}# Manage memory system${c.reset}`);
      console.log(`  omc m config      ${c.dim}# Manage configuration${c.reset}`);
      console.log(`  omc m statusline  ${c.dim}# Manage statusline${c.reset}`);
      console.log(`  omc m style       ${c.dim}# Manage output styles${c.reset}`);
      console.log(`  omc m ollama      ${c.dim}# Manage Ollama embedding models for memory${c.reset}`);
      console.log(`  omc m tc          ${c.dim}# Write terminal configuration (tmux, wezterm)${c.reset}`);
      console.log(`  omc m cleanup     ${c.dim}# Clean stale session data and temp files${c.reset}`);
      console.log(`  omc m codex log   ${c.dim}# View Codex daemon activity log${c.reset}`);
      console.log();
      console.log(`Run ${c.cyan}omc m <subcommand> --help${c.reset} for details.`);
    });
