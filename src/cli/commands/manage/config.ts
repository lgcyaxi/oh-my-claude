/**
 * CLI "config" command — Manage oh-my-claude omc configuration
 *
...
export function registerConfigCommand(program: Command) {
  program
    .command("config")
    .description("Manage oh-my-claude omc configuration")
    ...
    .action((options) => {
      ...
      if (options.get) {
        ...
        if (key in config) {
          ...
          console.log(
            `Run 'oh-my-claude omc config --list' to see all keys.`
          );
          process.exit(1);
        }
        return;
      ...
      console.log(
        `${c.bold}Manage oh-my-claude omc configuration${c.reset}\n`
      );
      console.log("Usage:");
      console.log("  oh-my-claude omc config --list");
      console.log("  oh-my-claude omc config --get <key>");
      console.log("  oh-my-claude omc config --set <key>=<value>");
      console.log("  oh-my-claude omc config --unset <key>");
      console.log(
        `\nRun 'oh-my-claude omc config --list' to see available keys.`
      );
    });
}
