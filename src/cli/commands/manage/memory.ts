...
export function registerMemoryCommand(program: Command) {
  ...
  const memoryCmd = program
    .command("memory")
    .description("Manage oh-my-claude omc memory system")
    .action(() => {
      ...
      console.log(`\nUsage:`);
      console.log(`  oh-my-claude omc memory status               ${c.dim}# Show memory stats${c.reset}`);
      console.log(`  oh-my-claude omc memory scope [mode]         ${c.dim}# Get/set storage scope (project|global|auto)${c.reset}`);
      console.log(`  oh-my-claude omc memory search <query>       ${c.dim}# Search memories${c.reset}`);
      console.log(`  oh-my-claude omc memory list [--scope all]   ${c.dim}# List memories${c.reset}`);
      console.log(`  oh-my-claude omc memory show <id>            ${c.dim}# Show memory content${c.reset}`);
      console.log(`  oh-my-claude omc memory delete <id>          ${c.dim}# Delete a memory${c.reset}`);
      console.log(`  oh-my-claude omc memory compact              ${c.dim}# Compact memories (interactive)${c.reset}`);
    });
  ...
  memoryCmd
    ...
    .action((options: { scope: string }) => {
      ...
      console.log(`  ${c.cyan}2.${c.reset} Or use MCP tool: ${c.green}mcp__oh-my-claude__compact_memories${c.reset}`);
      console.log();
      console.log(`${c.dim}Compaction uses AI (Z.AI/ZhiPu -> MiniMax -> DeepSeek) to analyze and group related memories.${c.reset}`);
    });

  // Memory scope subcommand
  memoryCmd
    .command("scope [mode]")
    .description("Get or set memory storage scope (project, global, auto)")
    .action((mode?: string) => {
      const configPath = join(homedir(), ".claude", "oh-my-claude.json");
      const { getProjectMemoryDir, getDefaultWriteScope } = require("../../../memory");
      const projectDir = getProjectMemoryDir();
      ...
      const currentWrite = memConfig.defaultWriteScope ?? "auto";
      const currentRead = memConfig.defaultReadScope ?? "all";
      const effectiveWrite = getDefaultWriteScope();

      // No argument: show current scope
      if (!mode) {
        console.log(`${c.bold}Memory Scope${c.reset}\n`);
        console.log(`  Write scope:     ${c.green}${currentWrite}${c.reset} ${currentWrite === "auto" ? c.dim + `(effective: ${effectiveWrite})` + c.reset : ""}`);
        console.log(`  Read scope:      ${c.green}${currentRead}${c.reset}`);
        console.log(`  Project dir:     ${projectDir ? c.dim + projectDir + c.reset : c.yellow + "(not in git repo)" + c.reset}`);
        console.log();
        console.log(`${c.bold}Usage:${c.reset}`);
        console.log(`  oh-my-claude omc memory scope project   ${c.dim}# Store new memories in .claude/mem/${c.reset}`);
        console.log(`  oh-my-claude omc memory scope global    ${c.dim}# Store new memories in ~/.claude/oh-my-claude/memory/${c.reset}`);
        console.log(`  oh-my-claude omc memory scope auto      ${c.dim}# Auto-detect: project if in git repo, else global${c.reset}`);
        return;
      }

      // Validate mode
      const validModes = ["project", "global", "auto"];
      if (!validModes.includes(mode)) {
        console.log(fail(`Invalid scope: "${mode}". Must be one of: ${validModes.join(", ")}`));
        process.exit(1);
      }

      // Check project availability
      if (mode === "project" && !projectDir) {
        console.log(warn(`Not in a git repository. "project" scope won't work until you're inside one.`));
      }

      // Update config
      if (!config.memory) config.memory = {};
      config.memory.defaultWriteScope = mode;
      // Read scope: "project" means only project, "global" means only global, "auto" → "all"
      ...
      console.log(`  ${c.dim}Read scope: ${config.memory.defaultReadScope}${c.reset}`);
    });
}
