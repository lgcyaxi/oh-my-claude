...
export function registerStatuslineCommand(program: Command) {
	...
	statuslineCmd
		...
		.description(
			'Toggle a segment on/off (model, git, directory, context, session, output-style, memory, mode, proxy, bridge, usage, preferences, codex)',
		)
		.action((segment: string, state?: string) => {
			...
			try {
				...
				const newState = config.segments[segment]?.enabled
					? 'enabled'
					: 'disabled';
				console.log(
					`${c.green}✓${c.reset} Segment "${segment}" ${newState}`,
				);
			} catch (error) {
				console.log(
