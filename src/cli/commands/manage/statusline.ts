...
export function registerStatuslineCommand(program: Command) {
  ...
  const statuslineCmd = program
    ...
    .action((options) => {
      ...
      if (options.status || (!options.enable && !options.disable)) {
        ...
        // Enable statusline
        const { installStatusLine } = require("../../../integration/installer/settings-merger");
        const { getStatusLineScriptPath } = require("../../../integration/installer");
        ...
        // Disable statusline
        const { uninstallStatusLine } = require("../../../integration/installer/settings-merger");
  ...
  statuslineCmd
    ...
    .action((name: string) => {
      ...
      const { setPreset } = require("../../../integration/statusline/config");
  ...
  statuslineCmd
    ...
    .action((segment: string, state?: string) => {
      ...
      const { toggleSegment, loadConfig } = require("../../../integration/statusline/config");
