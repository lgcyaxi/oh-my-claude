    ...
    .action(async (options: { dev?: boolean; build?: boolean }) => {
      ...
      if (options.build) {
        ...
        try {
          ...
          const builtExe = join(menubarDir, "src-tauri", "target", "release",
            process.platform === "win32" ? "omc-menubar.exe" : "omc-menubar");
          if (existsSync(builtExe)) {
            const platformDir = process.platform === "win32" ? "windows-x64"
              : process.platform === "darwin" ? `macos-${process.arch === "arm64" ? "arm64" : "x64"}`
              : "linux-x64";
            const buildsDir = join(menubarDir, "builds", platformDir);
            mkdirSync(buildsDir, { recursive: true });
            cpSync(builtExe, join(buildsDir, process.platform === "win32" ? "omc-menubar.exe" : "omc-menubar"));
            console.log(ok(`Copied to builds/${platformDir}/`));
          }
      ...
      if (options.dev) {
        ...
        if (isMac) {
          // macOS: look for pre-built binary first, then local build
          ...
          // Windows: look for pre-built binary first, then local build
          const possiblePaths = [
            join(menubarDir, "builds", "windows-x64", "omc-menubar.exe"),
          ...
          // Linux: look for pre-built binary first, then local build
          const possiblePaths = [
            join(menubarDir, "builds", "linux-x64", "omc-menubar"),
            join(menubarDir, "src-tauri", "target", "release", "omc-menubar"),
