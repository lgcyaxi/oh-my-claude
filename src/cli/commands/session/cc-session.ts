...
import { spawnSessionProxy, spawnDetachedProxy } from "../../utils/proxy-lifecycle";
import { launchInWezterm, launchInTmux, shouldUseTmuxInline } from "./cc-terminals";
import type { DetectedTerminal } from "../../utils/terminal-detect";
...
export async function launchDetachedSession(options: {
  ...
  const { sessionId, ports, terminal, claudeArgs, debug, bridgeMode } = options;
  ...
  if (terminal === "wezterm") {
    paneId = launchInWezterm(baseUrl, ports.controlPort, claudeArgsStr, debug, cwd, proxyPid, bridgeMode);
  } else if (terminal === "tmux") {
    paneId = launchInTmux(sessionId, baseUrl, ports.controlPort, claudeArgsStr, debug, cwd, proxyPid, bridgeMode);
  }
...
export async function launchInlineSession(options: {
  ...
  const { sessionId, ports, claudeArgs, debug, bridgeMode, isRemoteControl, terminalMode, insideWezTerm } = options;
  const { c, ok, fail, dimText } = createFormatters();
  ...
  if (isRemoteControl) {
    ...
    console.log(dimText("  Connect from claude.ai/code or Claude mobile app"));
    console.log(dimText("  API calls will route through the oh-my-claude proxy"));
    console.log(dimText("  Use /omc-switch in-session to change provider\n"));
  ...
  if (useTmuxInline) {
    ...
    const result = spawnSync("tmux", ["new-session", "-s", tmuxSession, shellCmd], {
      stdio: "inherit",
      env: process.env,
    });
  ...
  if (bridgeMode && (process.env.OMC_BRIDGE_PANE || "").trim() === "1") {
    try {
      ...
      writeFileSync(signalPath, JSON.stringify({ sessionId, readyAt: Date.now() }), "utf-8");
    } catch {}
  }

  const isBridgeWorker = (process.env.OMC_BRIDGE_PANE || "").trim() === "1";
  const shell = isBridgeWorker && process.platform === "win32"
    ? "cmd.exe"
