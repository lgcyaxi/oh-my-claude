...
import {
	...
	getStateFilePath,
	type AIEntry,
} from '../../../workers/bridge/state';
import { getBridgeOrchestrator } from '../../../index';
import { getSystemTerminalBackend } from '../../../workers/terminal';
import type { AIConfig } from '../../../workers/daemon/types';
import {
	pollCCPaneResponse,
	pollForBridgeResponse,
	type PaneMonitorOptions,
} from '../../../workers/bridge/poll-response';
import { TmuxBackend } from '../../../workers/terminal/tmux';
import { WezTermBackend } from '../../../workers/terminal/wezterm';
import {
	readProxyRegistry,
	cleanupStaleEntries,
} from '../../../proxy/registry';
...
const DEFAULT_AI_CONFIGS: Record<string, Omit<AIConfig, 'name'>> = {
	codex: {
		// Proc-based daemon: communicates via JSON-RPC 2.0, no terminal pane
		cliCommand: 'codex',
		cliArgs: ['app-server'],
		idleTimeoutMs: 60000,
...
import { resolveAlias } from '../../../shared/providers/aliases';
...
function createAIConfig(name: string, overrides?: Partial<AIConfig>): AIConfig {
	...
	if (!defaults) {
		throw new Error(
			`Unknown AI: ${name}. Supported: ${Object.keys(DEFAULT_AI_CONFIGS).join(', ')}`,
		);
	}
...
export function registerBridgeCommand(program: Command) {
	...
	bridgeCmd
		.command('bus')
		.description('Manage the Bridge Bus server (reliable HTTP task broker)')
		.option('--stop', 'Stop the bus server')
		.option('--status', 'Show bus server status')
		.option('--port <port>', 'Bus server port', '18912')
		.action(
			async (options: {
				stop?: boolean;
				status?: boolean;
				...
				const { fail, ok, dimText, c } = createFormatters();
				const port = parseInt(options.port ?? '18912', 10);

				try {
					if (options.stop) {
						const { stopBus } =
							await import('../../../workers/bridge/bus/lifecycle');
						await stopBus(port);
						console.log(ok('Bridge Bus stopped'));
						return;
					}

					if (options.status) {
						const { checkBusHealth } =
							await import('../../../workers/bridge/bus/lifecycle');
						const health = await checkBusHealth(port);
						if (!health) {
							console.log(dimText('Bridge Bus is not running'));
							return;
						}
						console.log(`${c.bold}Bridge Bus Status${c.reset}`);
					...
					// Default: start the bus
					const { ensureBusRunning } =
						await import('../../../workers/bridge/bus/lifecycle');
					const result = await ensureBusRunning(port);
					if (result.started) {
						console.log(
							ok(`Bridge Bus started on port ${result.port}`),
						);
					} else {
						console.log(
							dimText(
								`Bridge Bus already running on port ${result.port}`,
							),
						);
					}
				} catch (error) {
	...
	bridgeCmd
		.command('up [ais...]')
		.description(
			'Start AI assistant(s): codex, opencode, gemini, cc, or all',
		)
		...
		.option(
			'--switch <alias>',
			'Auto-switch CC worker to a model after startup (e.g., ds, zp, mm)',
		)
		.action(
			async (
				ais: string[],
				...
				try {
					// Auto-start bus server if not running
					try {
						const { ensureBusRunning } =
							await import('../../../workers/bridge/bus/lifecycle');
						const busResult = await ensureBusRunning();
						if (busResult.started) {
							console.log(
								dimText(
									`Bridge Bus auto-started on port ${busResult.port}`,
								),
							);
						}
					} catch {
						// Non-critical — bus is optional for pane-based bridge
					}
					...
					if (paneRight && backend === 'tmux' && ccPaneId) {
						...
						const existingAI = state.ais.find(
							(ai) =>
								ai.paneId &&
								ai.terminalBackend === 'tmux' &&
								currentWindow.has(ai.paneId),
						);
					...
					if (paneRight && backend === 'wezterm' && ccPaneId) {
						...
						const existingAI = state.ais.find(
							(ai) =>
								ai.paneId && ai.terminalBackend === 'wezterm',
						);
						if (existingAI?.paneId) {
							firstRightPaneId = existingAI.paneId;
						}
					...
					for (const name of names) {
						...
						try {
							// Resolve --switch alias to provider/model BEFORE spawning
							// This passes the switch config via AIConfig so the proxy starts pre-switched
							// (avoids rate limit from post-spawn API switching)
							let switchProvider: string | undefined;
							let switchModel: string | undefined;
							if (
								options.switch &&
								getBaseAIName(name) === 'cc'
							) {
								const switchTarget = resolveAlias(
									options.switch,
								);
								if (switchTarget) {
									switchProvider = switchTarget.provider;
									switchModel = switchTarget.model;
								} else {
									console.log(
										dimText(
											`  Warning: Unknown switch alias: ${options.switch}`,
										),
									);
								}
							}
							...
							const projectPath =
								daemon.getProjectPath?.() ?? process.cwd();
							...
							const switchInfo =
								switchProvider && switchModel
									...
									: '';
							console.log(
								ok(`Started ${name}${paneInfo}${switchInfo}`),
							);

							// For CC workers: wait for proxy to register and for bridge-ready signal.
							// Phase 1 (≤10s): poll proxy registry to get sessionId + controlPort.
							// Phase 2 (≤20s extra): poll bridge-ready-{sessionId}.json written by the
							//   memory-awareness hook when CC's first UserPromptSubmit fires.
							if (getBaseAIName(name) === 'cc') {
								...
								// Phase 1: wait for proxy registration
								...
								if (controlPort && sessionId) {
									const updatedState = readBridgeState();
									const aiEntry = updatedState.ais.find(
										(a) => a.name === name,
									);
									if (aiEntry) {
										aiEntry.proxySessionId = sessionId;
										aiEntry.proxyControlPort = controlPort;
										writeBridgeState(updatedState);
									}

									// Phase 2: wait for bridge-ready signal written by `oh-my-claude cc`
									// just before it launches Claude Code. Gives higher confidence that
									// the CC process is actually running, not just the proxy.
									const readySignal = join(
										homedir(),
										'.claude',
										'oh-my-claude',
										'run',
										`bridge-ready-${sessionId}.json`,
									);
									const readyMaxMs = 20_000;
									const readyStart = Date.now();
									while (
										Date.now() - readyStart <
										readyMaxMs
									) {
										if (existsSync(readySignal)) break;
										await new Promise((resolve) =>
											setTimeout(resolve, pollIntervalMs),
										);
									}
	...
	bridgeCmd
		.command('down [ais...]')
		.description(
			'Stop AI assistant(s): codex, opencode, gemini, cc, or all',
		)
	...
	bridgeCmd
		...
		.option(
			'--no-auto-close',
			'Keep pane alive after receiving response (default: auto-close)',
		)
		.action(
			async (
				ai: string,
				...
				try {
					...
					if (!entry) {
						console.log(
							fail(
								`${name} is not running. Start it with: oh-my-claude bridge up ${name}`,
							),
						);
						process.exit(1);
					}
					...
					if (!entry.paneId) {
						// Proc-based daemons (e.g., codex) have no pane — use MCP bridge_send tool instead
						const isCodex = getBaseAIName(name) === 'codex';
						if (isCodex) {
							console.log(
								fail(
									`${name} is a proc-based daemon (no terminal pane). Use the bridge_send MCP tool to send tasks to it.`,
								),
							);
						} else {
							console.log(
								fail(
									`${name} has no pane ID. It may have been started with an older version. Try: bridge down ${name} && bridge up ${name}`,
								),
							);
						}
						process.exit(1);
					...
					if (
						entry.terminalBackend !== 'wezterm' &&
						entry.terminalBackend !== 'tmux'
					) {
						console.log(
							fail(
								`bridge send requires WezTerm or tmux backend (got: ${entry.terminalBackend ?? 'unknown'})`,
							),
						);
						process.exit(1);
					}
					...
					console.log(
						dimText(
							`Sending to ${name} via ${entry.terminalBackend}...`,
						),
					);
					...
					if (
						...
						console.log(
							dimText(
								`Response polling not supported for ${name} yet`,
							),
						);
						return;
					}
					...
					const timeoutMs =
						parseInt(options.timeout ?? '120000', 10) || 120000;
					const projectPath = entry.projectPath ?? process.cwd();

					console.log(
						dimText(
							`Waiting for response (timeout: ${Math.round(timeoutMs / 1000)}s)...`,
						),
					);
					...
					if (baseName === 'cc') {
						...
						response = await pollCCPaneResponse(
							entry,
							message,
							timeoutMs,
						);
						...
						let paneMonitor: PaneMonitorOptions | undefined;
			...
			try {
				...
				if (
					...
					if (alive) {
						console.log(
							ok(
								`${name} pane ${entry.paneId} is alive (${entry.terminalBackend})`,
							),
						);
					} else {
						console.log(
							warn(
								`${name} pane ${entry.paneId} is no longer alive`,
							),
						);
						process.exit(1);
					}
		...
		.action(async (ai: string, options: { lines?: string }) => {
			...
			try {
				...
				if (!logPath) {
					...
					console.log(
						dimText(
							`Expected logs under ${join(INSTALL_DIR, 'run', name, 'logs')}`,
						),
					);
					process.exit(1);
				}
				...
				const tail = fileLines.slice(
					Math.max(0, fileLines.length - lines),
				);

				console.log(
					`${c.bold}Logs: ${name}${c.reset} ${c.dim}(${logPath})${c.reset}`,
				);
				if (tail.length === 0) {
...
function printBridgeUsage(): void {
	...
	console.log(
		`  oh-my-claude bridge up [codex|opencode|gemini|cc|all]   ${dimText('# Start AI assistant(s)')}`,
	);
	console.log(
		`  oh-my-claude bridge down [codex|opencode|gemini|cc|all] ${dimText('# Stop AI assistant(s)')}`,
	);
	console.log(
		`  oh-my-claude bridge send <ai> "<message>"              ${dimText('# Send message to AI')}`,
	);
	console.log(
		`  oh-my-claude bridge status                               ${dimText('# Show all AI statuses')}`,
	);
	console.log(
		`  oh-my-claude bridge ping <ai-name>                       ${dimText('# Check AI health')}`,
	);
	console.log(
		`  oh-my-claude bridge logs <ai-name>                       ${dimText('# Show recent AI logs')}`,
	);
	console.log(
		`  oh-my-claude bridge bus                                  ${dimText('# Start Bridge Bus server')}`,
	);
	console.log(
		`  oh-my-claude bridge bus --stop                           ${dimText('# Stop Bridge Bus server')}`,
	);
	console.log(
		`  oh-my-claude bridge bus --status                         ${dimText('# Show Bridge Bus status')}`,
	);

	console.log(`\n${c.bold}Examples${c.reset}`);
	console.log(
		`  oh-my-claude bridge up codex                        ${dimText('# Opens alongside Claude Code (right pane)')}`,
	);
	console.log(
		`  oh-my-claude bridge up cc                           ${dimText('# Spawn CC with own proxy session')}`,
	);
	console.log(
		`  oh-my-claude bridge up cc --switch ds               ${dimText('# CC auto-switched to DeepSeek')}`,
	);
	console.log(
		`  oh-my-claude bridge up cc cc:2 cc:3                 ${dimText('# Multiple independent CC instances')}`,
	);
	console.log(
		`  oh-my-claude bridge up codex --detached             ${dimText('# Opens in separate window')}`,
	);
	...
	console.log(
		`  oh-my-claude bridge bus                             ${dimText('# Start reliable HTTP task broker')}`,
	);
 ...
 */
/** Aliases that map to canonical AI names */
const AI_NAME_ALIASES: Record<string, string> = {
...
function normalizeAIName(input: string): string {
	...
	// Resolve aliases (e.g., "codex-app-server" → "codex")
	const resolved = AI_NAME_ALIASES[value] ?? value;
	const baseName = getBaseAIName(resolved);
	...
	throw new Error(
		`Unknown AI name "${input}". Use one of: ${KNOWN_AI_NAMES.join(', ')}, all, or cc:N for multiple CC instances`,
	);
}
...
async function loadBridge(): Promise<BridgeLike> {
	...
	if (!isBridgeLike(instance)) {
		throw new Error(
			'Bridge core export is invalid. Expected methods: registerAI, unregisterAI, ping, getSystemStatus.',
		);
	}
...
function printBridgeStatus(
	...
	const widthName = Math.max(
		header[0].length,
		...ais.map((ai) => ai.name.length),
	);
	const widthCmd = Math.max(
		header[1].length,
		...ais.map((ai) => ai.cliCommand.length),
	);
	const widthPane = Math.max(
		header[2].length,
		...ais.map((ai) => (ai.paneId ?? '-').length),
	);
	const widthBackend = Math.max(
		header[3].length,
		...ais.map((ai) => (ai.terminalBackend ?? '-').length),
	);
...
function sendToWezTermPane(paneId: string, text: string): void {
	...
	if (textResult.error) {
		throw new Error(
			`Failed to send text to WezTerm pane ${paneId}: ${textResult.error.message}`,
		);
	}
	if (textResult.status !== 0) {
		throw new Error(
			`wezterm cli send-text exited with code ${textResult.status}`,
		);
	}
	...
	if (enterResult.error) {
		throw new Error(
			`Failed to send Enter to WezTerm pane ${paneId}: ${enterResult.error.message}`,
		);
	}
}
	...
	try {
		if (!firstRightPaneId) {
			...
			if (result.status !== 0) {
				...
				console.log(
					dimText(
						`  Layout: join-pane -h failed (${stderr}), keeping as separate window`,
					),
				);
				return paneId;
			}
			...
			if (result.status !== 0) {
				...
				console.log(
					dimText(
						`  Layout: join-pane -v failed (${stderr}), keeping as separate window`,
					),
				);
				return paneId;
			}
...
function sendToTmuxPane(paneId: string, text: string): void {
	...
	if (cleanText.includes('\n')) {
		...
		if (bufResult.status === 0) {
			...
			const textResult = spawnSync(
				'tmux',
				['send-keys', '-t', paneId, '-l', cleanText],
				{
					timeout: 10_000,
				},
			);
			if (textResult.status !== 0) {
				...
				throw new Error(
					`tmux send-keys exited with code ${textResult.status}: ${stderr}`,
				);
			}
		}
	} else {
		...
		if (textResult.error) {
			throw new Error(
				`Failed to send text to tmux pane ${paneId}: ${textResult.error.message}`,
			);
		}
		if (textResult.status !== 0) {
			const stderr = textResult.stderr?.toString() ?? '';
	...
	if (enterResult.error) {
		throw new Error(
			`Failed to send Enter to tmux pane ${paneId}: ${enterResult.error.message}`,
		);
	}
}
...
function formatBridgeError(aiName: string, error: unknown): string {
	...
	const baseName = getBaseAIName(aiName);
	if (
		normalized.includes('cli_not_found') ||
