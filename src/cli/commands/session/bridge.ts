...
import { pollCCPaneResponse, pollForBridgeResponse, type PaneMonitorOptions } from "../../../workers/bridge/poll-response";
...
/** Switch alias → { provider, model } mapping for direct proxy API switching */
const SWITCH_ALIAS_MAP: Record<string, { provider: string; model: string }> = {
...
export function registerBridgeCommand(program: Command) {
  ...
  bridgeCmd
    ...
    .action(async (options: { stop?: boolean; status?: boolean; port?: string }) => {
      const { fail, ok, dimText, c } = createFormatters();
      const port = parseInt(options.port ?? "18912", 10);

      try {
        if (options.stop) {
          const { stopBus } = await import("../../../workers/bridge/bus/lifecycle");
          await stopBus(port);
        ...
        if (options.status) {
          const { checkBusHealth } = await import("../../../workers/bridge/bus/lifecycle");
        ...
        const { ensureBusRunning } = await import("../../../workers/bridge/bus/lifecycle");
        const result = await ensureBusRunning(port);
        if (result.started) {
          console.log(ok(`Bridge Bus started on port ${result.port}`));
        } else {
          console.log(dimText(`Bridge Bus already running on port ${result.port}`));
        }
      } catch (error) {
      ...
      try {
        ...
        try {
          ...
          if (busResult.started) {
            console.log(dimText(`Bridge Bus auto-started on port ${busResult.port}`));
          }
        } catch {
        ...
        for (const name of names) {
          ...
          try {
            ...
            if (options.switch && getBaseAIName(name) === "cc") {
              const switchTarget = SWITCH_ALIAS_MAP[options.switch];
              if (switchTarget) {
                switchProvider = switchTarget.provider;
                switchModel = switchTarget.model;
              } else {
                console.log(dimText(`  Warning: Unknown switch alias: ${options.switch}`));
              }
            }
            ...
            const switchInfo = switchProvider && switchModel
              ? ` → ${switchProvider}/${switchModel}`
              : "";
            console.log(ok(`Started ${name}${paneInfo}${switchInfo}`));
            ...
            if (getBaseAIName(name) === "cc") {
              ...
              if (controlPort && sessionId) {
                ...
                const aiEntry = updatedState.ais.find((a) => a.name === name);
                if (aiEntry) {
                ...
                const readySignal = join(
                  homedir(), ".claude", "oh-my-claude", "run",
                  `bridge-ready-${sessionId}.json`,
                );
                const readyMaxMs = 20_000;
                const readyStart = Date.now();
                while (Date.now() - readyStart < readyMaxMs) {
    ...
    .action(async (ai: string, message: string, options: { timeout?: string; wait?: boolean; autoClose?: boolean }) => {
      ...
      try {
        ...
        if (!entry.paneId) {
          ...
          if (isCodex) {
            console.log(fail(`${name} is a proc-based daemon (no terminal pane). Use the bridge_send MCP tool to send tasks to it.`));
          } else {
...
function printBridgeUsage(): void {
  ...
  console.log(`  oh-my-claude bridge bus                                  ${dimText("# Start Bridge Bus server")}`);
  console.log(`  oh-my-claude bridge bus --stop                           ${dimText("# Stop Bridge Bus server")}`);
  console.log(`  oh-my-claude bridge bus --status                         ${dimText("# Show Bridge Bus status")}`);
  ...
  console.log(`  oh-my-claude bridge bus                             ${dimText("# Start reliable HTTP task broker")}`);
