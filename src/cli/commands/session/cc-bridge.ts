...
import { DEFAULT_BRIDGE_AI_CONFIGS } from '../../../mcp/bridge/config';
import type {
	BridgeWorkerDefinition,
	WorkerRole,
} from '../../../assets/agents/types';
import {
	loadConfig,
...
const resolveWorkerMetadata = (
	...
	const config =
		...
		(workerName === 'cc:glm'
			? DEFAULT_BRIDGE_AI_CONFIGS['cc:zp']
			: undefined) ??
		(workerName === 'cc:mm'
			? DEFAULT_BRIDGE_AI_CONFIGS['cc:mm-cn']
			: undefined);
...
export function resolveBridgeWorkers(): ResolveWorkersResult {
	...
	try {
		...
		if (process.env.KIMI_API_KEY) {
			workers.push({
				name: 'cc:kimi',
				switchAlias: 'kimi',
				label: 'Kimi K2.5 (code)',
				...resolveWorkerMetadata('cc:kimi'),
			});
		}
		if (process.env.MINIMAX_API_KEY) {
			workers.push({
				name: 'cc:mm',
				switchAlias: 'mm',
				label: 'MiniMax-M2.5 (docs)',
				...resolveWorkerMetadata('cc:mm'),
			});
		} else if (process.env.MINIMAX_CN_API_KEY) {
			workers.push({
				name: 'cc:mm',
				switchAlias: 'mm-cn',
				label: 'MiniMax-M2.5 CN (docs)',
				...resolveWorkerMetadata('cc:mm'),
			});
		}
		if (process.env.ZAI_API_KEY) {
			workers.push({
				name: 'cc:glm',
				switchAlias: 'zai',
				label: 'GLM-5 via Z.AI (research)',
				...resolveWorkerMetadata('cc:glm'),
			});
	...
	if (!config.providers?.minimax && !config.providers?.['minimax-cn']) {
		...
		isProviderConfigured(config, 'minimax') &&
		hasReachableBaseUrl(config, 'minimax')
	) {
		workers.push({
			name: 'cc:mm',
			switchAlias: 'mm',
		...
		isProviderConfigured(config, 'minimax-cn') &&
		hasReachableBaseUrl(config, 'minimax-cn')
	) {
		workers.push({
			name: 'cc:mm',
			switchAlias: 'mm-cn',
	...
	if (!config.providers?.zai && !config.providers?.zhipu) {
		addSkipped('cc:glm', 'provider not configured');
	} else if (
		isProviderConfigured(config, 'zai') &&
		hasReachableBaseUrl(config, 'zai')
	) {
		workers.push({
			name: 'cc:glm',
			switchAlias: 'zai',
		...
		isProviderConfigured(config, 'zhipu') &&
		hasReachableBaseUrl(config, 'zhipu')
	) {
		workers.push({
			name: 'cc:glm',
			switchAlias: 'zp',
	...
	for (const worker of workers) {
		try {
			...
			if (!lockResult.added) {
				...
				spawnFailed.push({
					name: worker.name,
					reason: lockResult.error ?? 'already running',
				});
			} else {
				spawned.push(`${worker.name} (${worker.role})`);
				console.log(
					dimText(
						`  Spawned: ${c.cyan}${worker.name}${c.reset} [${worker.switchAlias}]`,
					),
				);
			}
		} catch (err) {
