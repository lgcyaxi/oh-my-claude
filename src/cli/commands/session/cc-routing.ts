...
/** Provider alias → { name, baseUrl, apiKeyEnv, defaultModel? } */
export const PROVIDER_MAP: Record<string, { name: string; baseUrl: string; apiKeyEnv: string; defaultModel?: string }> = {
  ds:           { name: "DeepSeek",      baseUrl: "https://api.deepseek.com/anthropic",                    apiKeyEnv: "DEEPSEEK_API_KEY",    defaultModel: "deepseek-chat" },
  deepseek:     { name: "DeepSeek",      baseUrl: "https://api.deepseek.com/anthropic",                    apiKeyEnv: "DEEPSEEK_API_KEY",    defaultModel: "deepseek-chat" },
  zp:           { name: "ZhiPu",         baseUrl: "https://open.bigmodel.cn/api/anthropic",                apiKeyEnv: "ZHIPU_API_KEY",       defaultModel: "glm-5" },
  zhipu:        { name: "ZhiPu",         baseUrl: "https://open.bigmodel.cn/api/anthropic",                apiKeyEnv: "ZHIPU_API_KEY",       defaultModel: "glm-5" },
  zai:          { name: "Z.AI",          baseUrl: "https://api.z.ai/api/anthropic",                        apiKeyEnv: "ZAI_API_KEY",         defaultModel: "glm-5" },
  "zp-g":       { name: "Z.AI",          baseUrl: "https://api.z.ai/api/anthropic",                        apiKeyEnv: "ZAI_API_KEY",         defaultModel: "glm-5" },
  mm:           { name: "MiniMax",       baseUrl: "https://api.minimax.io/anthropic",                      apiKeyEnv: "MINIMAX_API_KEY",     defaultModel: "MiniMax-M2.5" },
  minimax:      { name: "MiniMax",       baseUrl: "https://api.minimax.io/anthropic",                      apiKeyEnv: "MINIMAX_API_KEY",     defaultModel: "MiniMax-M2.5" },
  "mm-cn":      { name: "MiniMax CN",    baseUrl: "https://api.minimaxi.com/anthropic",                    apiKeyEnv: "MINIMAX_CN_API_KEY",  defaultModel: "MiniMax-M2.5" },
  "minimax-cn": { name: "MiniMax CN",    baseUrl: "https://api.minimaxi.com/anthropic",                    apiKeyEnv: "MINIMAX_CN_API_KEY",  defaultModel: "MiniMax-M2.5" },
  km:           { name: "Kimi",          baseUrl: "https://api.kimi.com/coding",                            apiKeyEnv: "KIMI_API_KEY",        defaultModel: "kimi-for-coding" },
  kimi:         { name: "Kimi",          baseUrl: "https://api.kimi.com/coding",                            apiKeyEnv: "KIMI_API_KEY",        defaultModel: "kimi-for-coding" },
  ay:           { name: "Aliyun",        baseUrl: "https://coding.dashscope.aliyuncs.com/apps/anthropic",  apiKeyEnv: "ALIYUN_API_KEY",      defaultModel: "qwen3.5-plus" },
  aliyun:       { name: "Aliyun",        baseUrl: "https://coding.dashscope.aliyuncs.com/apps/anthropic",  apiKeyEnv: "ALIYUN_API_KEY",      defaultModel: "qwen3.5-plus" },
};
...
export function detectTerminalBackend(): TerminalBackend {
  ...
  try {
    const whichCmd = process.platform === "win32" ? "where wezterm" : "which wezterm";
...
export function isBridgeModeActive(): boolean {
  ...
  try {
    const statePath = join(homedir(), ".claude", "oh-my-claude", "bridge-state.json");
    if (!existsSync(statePath)) return false;
    const parsed = JSON.parse(readFileSync(statePath, "utf-8")) as { ais?: unknown[] };
...
export function isProxyConfigured(): boolean {
  if ((process.env.OMC_PROXY_CONTROL_PORT || "").trim().length > 0) return true;
...
export function resolveProxySwitchAlias(providerOption?: string): string {
  const alias = (providerOption || process.env.OMC_SWITCH_ALIAS || "revert").trim().toLowerCase();
...
export function spawnProxySwitchedCC(
  ...
  const buildSwitchCmd = (alias: string): string => {
    ...
    const providerMap: Record<string, string> = {
      ds: "deepseek", "ds-r": "deepseek", zp: "zhipu", zai: "zhipu-global",
      mm: "minimax", "mm-cn": "minimax-cn", km: "kimi", ay: "aliyun", ol: "ollama",
    };
    const provider = providerMap[alias] ?? alias;
  ...
  try {
    if (terminal === "tmux") {
      ...
      const result = spawnSync("tmux", tmuxArgs, { stdio: "ignore", windowsHide: true });
    ...
    if (terminal === "wezterm") {
      ...
      if (process.platform === "win32") {
        const switchPart = alias !== "revert" ? `oh-my-claude proxy switch ${alias} 2>nul & ` : "";
      ...
      const result = spawnSync(
        wezterm,
        ["cli", "spawn", "--cwd", cwd, "--", "bash", "-lc", unixCommand],
        { stdio: "ignore", windowsHide: true },
      );
      return result.status === 0;
    }
...
export function spawnMacOSTerminal(alias: string, claudeArgs: string[], cwd: string, controlPort?: number): boolean {
  ...
  const provider = providerMap[alias] ?? alias;
  const switchPart = (alias && alias !== "revert")
  ...
  const result = spawnSync("osascript", ["-e", osaScript], { stdio: "ignore", windowsHide: true });
  return result.status === 0;
}
