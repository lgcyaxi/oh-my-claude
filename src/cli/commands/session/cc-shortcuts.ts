 ...
 * Returns { args, isRemoteControl, worktreeName, bridgeMode } where:
 * - isRemoteControl triggers `claude remote-control` mode
 * - worktreeName is set when -wt [name] was used (triggers pre-creation)
 */
export function expandShortcuts(claudeArgs: string[]): {
  ...
  return { args: expanded, isRemoteControl, worktreeName, bridgeMode };
}
