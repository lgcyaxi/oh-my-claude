...
import { readProxyRegistry, unregisterProxySession, cleanupStaleEntries } from "../../../proxy/registry";
import { detectTerminal } from "../../utils/terminal-detect";

import { expandShortcuts, preCreateWorktree } from "./cc-shortcuts";
import { PROVIDER_MAP, detectTerminalBackend, isBridgeModeActive, runNativeCC, loadApiEnvFile, formatAge } from "./cc-routing";
...
export function registerCcCommand(program: Command) {
  const ccCmd = program
    ...
    .option("-p, --provider <alias>", "Connect directly to provider (ds/zp/zai/mm/mm-cn/km/ay)")
...
Examples:
    ...
    .action((sessionId?: string) => {
      ...
      try {
        if (process.platform === "win32") {
          execSync(`taskkill /F /PID ${target.pid}`, { stdio: "ignore", windowsHide: true });
        } else {
      ...
      try {
        ...
        const modePath = join(homedir(), ".claude", "oh-my-claude", "sessions", target.sessionId, "mode.json");
        if (existsSync(modePath)) {
          const mode = JSON.parse(readFileSync(modePath, "utf-8"));
          if (mode.bridge) {
            console.log(dimText("Stopping bridge workers..."));
            try {
              execSync("oh-my-claude bridge down all", { stdio: "ignore", timeout: 10_000, windowsHide: true });
  ...
  ccCmd.action(async (rawClaudeArgs: string[], options) => {
    ...
    let { args: claudeArgs, isRemoteControl, worktreeName, bridgeMode } = expandShortcuts(rawClaudeArgs);
    ...
    // --- Provider pre-switch: route through proxy pre-switched to the specified provider ---
    // This replaces Mode A (direct bypass). The proxy starts pre-switched so:
    // - /model shows the provider's models (not Anthropic's)
    // - /omc-switch can change to another provider mid-session
    // - No direct API bypass needed
    if (options.provider) {
      ...
      if (!provider) {
        ...
        console.log(dimText(`Available: ds, zp, zai, mm, mm-cn, km, ay (or full names)`));
        process.exit(1);
      }
      ...
      // Tell the proxy to start pre-switched to this provider
      process.env.OMC_PROXY_SWITCH_PROVIDER = alias;
      process.env.OMC_PROXY_SWITCH_MODEL = provider.defaultModel ?? "";
      // NOTE: Do NOT pass --model to CC here. The proxy handles model switching via /switch endpoint.
      // Passing --model directly would bypass the proxy's switched provider.
      console.log(ok(`Provider: ${c.cyan}${provider.name}${c.reset} (pre-switched via proxy)`));
      if (provider.defaultModel) console.log(dimText(`  Default model: ${provider.defaultModel}`));
    }
    ...
    if (bridgeMode) {
      const sessionDir = join(homedir(), ".claude", "oh-my-claude", "sessions", sessionId);
      mkdirSync(sessionDir, { recursive: true });
      writeFileSync(join(sessionDir, "mode.json"), JSON.stringify({ ulw: false, bridge: true }));
      console.log(ok(`Bridge Mode: ${c.yellow}enabled${c.reset} (task delegation enforced)`));
    }
    ...
    if (terminalMode === "none" || insideWezTerm || isRemoteControl) {
      ...
      terminal = detected === "wezterm" || detected === "tmux" ? detected : null;
    }
    ...
    if (terminal) {
      ...
      if (bridgeMode) {
        ...
        spawnBridgeWorkers({
          ...
          console.log(dimText(`  Bridge worker spawn error: ${err instanceof Error ? err.message : String(err)}`));
        });
      }
    } else {
