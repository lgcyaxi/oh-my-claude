...
const OMC_STATUSLINE_COMMAND = join(
	homedir(),
	'.claude',
	'oh-my-claude',
	'dist',
...
function getNodePath(): string | null {
	...
	console.warn(
		"[statusline] Warning: Could not detect Node.js path. Using 'node' from PATH.",
	);
	_cachedNodePath = 'node';
	return _cachedNodePath;
}
...
export function mergeStatusLine(
	...
	if (isOurWrapper(existing.command)) {
		if (force) {
			// Check if the original command from backup still exists
			let originalCommand = '';
			if (existsSync(BACKUP_FILE_PATH)) {
				try {
					const backup = JSON.parse(
						readFileSync(BACKUP_FILE_PATH, 'utf-8'),
					);
					originalCommand = backup.command || '';
				} catch {
					// If backup is invalid, fall through to direct mode
				}
			}

			// Extract the script path from the original command to verify it exists
			const scriptPathMatch = originalCommand.match(
				/"([^"]+\.(sh|js|mjs|cjs))"/,
			);
			const scriptPath = scriptPathMatch?.[1];
			const originalStillValid = scriptPath && existsSync(scriptPath);
			...
			// Original command is gone — drop the wrapper, use direct statusline
			return {
				config: {
					type: 'command',
					command: buildNodeCommand(OMC_STATUSLINE_COMMAND),
...
export function validateStatusLineSetup(): {
	...
	if (existsSync(OMC_STATUSLINE_COMMAND)) {
		...
		errors.push(
			`Statusline script not found at: ${OMC_STATUSLINE_COMMAND}`,
		);
	}
	...
	if (platform() === 'win32') {
		...
		if (nodePath && nodePath !== 'node' && existsSync(nodePath)) {
			...
			warnings.push(
				"Using 'node' from PATH. Consider installing Node.js to a standard location.",
			);
			details.nodePathValid = true; // Still valid, just with a warning
		} else {
			errors.push(
				'Could not locate Node.js executable. Please ensure Node.js is installed.',
			);
		}
	} else {
		...
		try {
			...
			if (settings.statusLine?.command) {
				...
				if (isOurStatusLine(cmd) || isOurWrapper(cmd)) {
					...
					warnings.push(
						'settings.json has a different statusline configured',
					);
				}
			} else {
	...
	if (details.scriptExists && details.nodePathValid) {
		try {
			...
			if (error.status !== undefined) {
				...
				details.commandWorks = true;
				warnings.push(
					'Statusline command ran but returned non-zero exit code',
				);
			} else {
