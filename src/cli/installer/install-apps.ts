/**
 * CLI dist, proxy, WezTerm (LFS-aware), menubar (LFS-aware + download),
...
export async function installApps(ctx: InstallContext): Promise<void> {
	...
	try {
		const srcWeztermDir = join(ctx.sourceDir, 'apps', 'wezterm');
		const installWeztermDir = join(ctx.installDir, 'apps', 'wezterm');
