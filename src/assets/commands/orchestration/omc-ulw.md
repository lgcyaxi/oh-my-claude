...
Or combined: oh-my-claude cc -skip -bridge
```
...
1. **Write mode.json** to enable ULW statusline indicator:
   ...
   # Read existing mode.json to preserve bridge flag
   if [ -f ~/.claude/oh-my-claude/sessions/$SESSION_ID/mode.json ]; then
     BRIDGE=$(cat ~/.claude/oh-my-claude/sessions/$SESSION_ID/mode.json | grep -o '"bridge":\s*true' | wc -l)
   else
     BRIDGE=0
   fi

   if [ "$BRIDGE" -gt 0 ]; then
     echo '{"ulw":true,"bridge":true}' > ~/.claude/oh-my-claude/sessions/$SESSION_ID/mode.json
   else
     echo '{"ulw":true,"bridge":false}' > ~/.claude/oh-my-claude/sessions/$SESSION_ID/mode.json
   fi
   ```
   The statusline will now show `[ULW]` (or `[ULW+BRIDGE]` if bridge is also active).
...
| **Bridge Research Worker** | deepseek/deepseek-chat | Fast pattern review via `bridge_send(role=research, task)` |
| **Bridge Docs Worker** | zhipu/glm-5 | External docs and library research via `bridge_send(role=docs, task)` |
| **Frontend-UI-UX** | kimi/K2.5 | All frontend/UI work |
| **Bridge Docs Worker (Long-form)** | minimax/MiniMax-M2.5 | Documentation, README via `bridge_send(role=docs, task)` |
...
- Bridge docs worker for external documentation (`bridge_send(role=docs, task)`)
...
1. **Deactivate ULW mode** — write mode.json to clear ulw flag:
   ...
   if [ "$BRIDGE" -gt 0 ]; then
     echo '{"ulw":false,"bridge":true}' > ~/.claude/oh-my-claude/sessions/$SESSION_ID/mode.json
   else
     echo '{"ulw":false,"bridge":false}' > ~/.claude/oh-my-claude/sessions/$SESSION_ID/mode.json
   fi
   ```
