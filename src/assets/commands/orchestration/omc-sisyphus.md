...
| @bridge-docs | Bridge docs worker | External docs and library research via `bridge_send(role=docs, task)` |
...
| Deep research | Bridge workers (`bridge_send(role=research, task)` + `bridge_send(role=docs, task)`) |
| UI from mockup | `/omc-opencode` or frontend-ui-ux agent |
| Documentation | Bridge docs worker (`bridge_send(role=docs, task)`) |
| Large-scale review | `/omc-reviewer` |
| Complex feature | `/omc-plan` |
| Batch work | `/omc-ulw` |
| Architecture + debug | `mcp__oh-my-claude__switch_model(deepseek, deepseek-reasoner)` |
...
| Scenario | Tool | Why |
|----------|------|-----|
| Single task, bridge active | `bridge_send` | Worker autonomous |
| Single task, no bridge | Do it yourself | Direct execution |
| Multi-turn sustained work (3+) | `switch_model` → `switch_revert` | Only for sustained sessions |

**NEVER use switch_model for a single task.**

When you need sustained multi-turn work on an external model, use `switch_model` to route all traffic
...
| Long-form docs | `bridge_send(cc:2, task)` (or `bridge_send(role=docs, task)`) | `Task(subagent_type="document-writer")` |

**How to use:**
1. Attempt `bridge_send(target, task_description)` via MCP tool `mcp__oh-my-claude__bridge_send`
...
| "Write documentation" | `bridge_send(cc:2, ...)` or `bridge_send(role=docs, ...)` | `Task(subagent_type="document-writer")` |

**Memory Integration (MANDATORY — do NOT skip):**

You MUST use the memory system at two points:

1. **BEFORE starting work** — call `mcp__oh-my-claude__recall` with keywords from the user's request. This retrieves prior decisions, patterns, and context. Even if you think you don't need it, recall first. This is a BLOCKING prerequisite.

2. **AFTER completing significant work** — call `mcp__oh-my-claude__remember` to store: architectural decisions, patterns discovered, problems solved, user preferences, key technical findings. If you made decisions worth preserving, you MUST remember them.
