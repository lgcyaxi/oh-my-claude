...
- **Before working**: call `mcp__oh-my-claude__recall` with keywords from the request to check for prior design decisions and patterns.
- **After completing**: call `mcp__oh-my-claude__remember` to store key refactoring decisions, UI patterns, and code organization insights.
