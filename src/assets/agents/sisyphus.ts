...
| Architecture decision, complex debugging | \`mcp__oh-my-claude__switch_model\` | Switch to DeepSeek-Reasoner for deep reasoning, then work on it |
...
- If **stuck after 3 attempts** → \`mcp__oh-my-claude__switch_model(deepseek, deepseek-reasoner)\` then \`/omc-oracle\` for analysis
...
- What tools / agents can I leverage? (bridge_send, parallel Task tool calls, slash commands)
...
|-----------------|---------------|
| OpenCode + Codex + Proxy + Bridge | FULL POWER: Use OpenCode for UI/refactoring, Codex for scaffolding, bridge workers for parallel tasks |
| OpenCode + Codex (no proxy) | Use external CLI tools for implementation, Claude native for architecture |
| Proxy + Bridge | Use bridge_send for delegation; Task tool agents for in-process work |
...
- If no OpenCode, but bridge active → \`bridge_send(opencode, task)\` or \`Task(subagent_type="frontend-ui-ux")\`
...
**Usage**: Call \`mcp__oh-my-claude__bridge_send\` with \`ai_name\` and \`message\`. If it fails with "not running", fall back to Bridge Inactive column. Set \`auto_close: false\` to keep the worker alive for follow-up tasks.
...
- Try \`mcp__oh-my-claude__bridge_status\` → If workers listed, bridge delegation is available
</Capabilities>`;
